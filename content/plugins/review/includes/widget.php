...
class wp_review_tab_widget extends WP_Widget {
	function __construct() {
		...
		$widget_ops = array('classname' => 'widget_wp_review_tab', 'description' => __('Display Reviews in tabbed format.', 'review'));
		$control_ops = array('width' => 200, 'height' => 350);
		$this->WP_Widget('wp_review_tab_widget', __('WP Review Widget', 'review'), $widget_ops, $control_ops);
    }	
    function wp_review_tab_admin_scripts($hook) {
        wp_register_script('wp_review_tab_widget_admin', trailingslashit( WP_REVIEW_ASSETS ).'js/review-tab-widget-admin.js', array('jquery'), WP_REVIEW_PLUGIN_VERSION);  
        wp_enqueue_script('wp_review_tab_widget_admin');
    }
    function wp_review_tab_register_scripts() { 
		// JS
        wp_enqueue_script('jquery');
		wp_register_script('wp_review_tab_widget', trailingslashit( WP_REVIEW_ASSETS ).'js/review-tab-widget.js', array('jquery'), WP_REVIEW_PLUGIN_VERSION);     
		wp_localize_script( 'wp_review_tab_widget', 'wp_review_tab',         
			array( 'ajax_url' => admin_url( 'admin-ajax.php' )) 
		);        
		// CSS     
		wp_register_style('wp_review_tab_widget', trailingslashit( WP_REVIEW_ASSETS ).'css/review-tab-widget.css', array(), WP_REVIEW_PLUGIN_VERSION);
		wp_register_style( 'wp_review-style', trailingslashit( WP_REVIEW_ASSETS ) . 'css/review.css', array(), WP_REVIEW_PLUGIN_VERSION, 'all' );
	...
	function form( $instance ) {
        ...
        <h4><?php _e('Select Tabs', 'review'); ?></h4>
        ...
        <p class="wp_review_tab_review_type">
			<label for="<?php echo $this->get_field_id('review_type'); ?>"><?php _e('Review type:', 'review'); ?></label> 
			<select id="<?php echo $this->get_field_id('review_type'); ?>" name="<?php echo $this->get_field_name('review_type'); ?>" style="margin-left: 12px;">
				<option value="any" <?php selected($review_type, 'any', true); ?>><?php _e('Any', 'review'); ?></option>
				<option value="star" <?php selected($review_type, 'star', true); ?>><?php _e('Star', 'review'); ?></option>
				<option value="point" <?php selected($review_type, 'point', true); ?>><?php _e('Point', 'review'); ?></option>
				<option value="percentage" <?php selected($review_type, 'percentage', true); ?>><?php _e('Percentage', 'review'); ?></option>
			</select>       
		</p>
        ...
        <p>
			<label for="<?php echo $this->get_field_id("allow_pagination"); ?>">				
				...
				<?php _e( 'Allow pagination', 'review'); ?>
			</label>
		</p>

        <p>
			<label for="<?php echo $this->get_field_id('post_num'); ?>"><?php _e('Number of reviews to show:', 'review'); ?>
				<br />
		...
		<p class="wp_review_tab_thumbnail_size">
			<label for="<?php echo $this->get_field_id('thumb_size'); ?>"><?php _e('Thumbnail size:', 'review'); ?></label> 
			<select id="<?php echo $this->get_field_id('thumb_size'); ?>" name="<?php echo $this->get_field_name('thumb_size'); ?>" style="margin-left: 12px;">
				<option value="small" <?php selected($thumb_size, 'small', true); ?>><?php _e('Small', 'review'); ?></option>
				<option value="large" <?php selected($thumb_size, 'large', true); ?>><?php _e('Large', 'review'); ?></option>    
			</select>       
		</p>	
		...
		<p>			
			<label for="<?php echo $this->get_field_id("show_date"); ?>">	
				...
				<?php _e( 'Show date', 'review'); ?>	
			</label>	
		</p>
        
        <p class="wp_review_tab_custom_reviews"<?php echo (empty($tabs['custom']) ? ' style="display: none;"' : ''); ?>>
			<label for="<?php echo $this->get_field_id('custom_reviews'); ?>"><?php _e('Reviews on Custom tab:', 'review'); ?>
				<br />
        ...
        <h4><a href="#" class="wp_review_tab_titles_header"><?php _e('Tab Titles', 'review'); ?></a> | <a href="#" class="wp_review_tab_order_header"><?php _e('Tab Order', 'review'); ?></a></h4>
	...
	function widget( $args, $instance ) {	
		...
		<div class="wp_review_tab_widget_content" id="<?php echo $widget_id; ?>_content">		
			<ul class="review-tabs <?php echo "has-$tabs_count-"; ?>tabs">
				...
				<li>
					<a title="<?php the_title(); ?>" rel="nofollow" href="<?php the_permalink() ?>">		
						<div class="wp_review_tab_thumbnail wp_review_tab_thumb_<?php echo $thumb_size; ?>">	
							...
							<?php else: ?>
								<img src="<?php echo WP_REVIEW_ASSETS.'images/'.$thumb_size.'thumb.png'; ?>" alt="<?php the_title(); ?>" class="post-image" />		
							<?php endif; ?>
						</div>
					</a>
    ...
    function tab_pagination($page, $last_page) {  
		?>   
		<div class="review-tab-pagination">     
			<?php if ($page > 1) : ?>               
				<a href="#" class="previous"><span><?php _e('&laquo; Previous', 'review'); ?></span></a>      
			<?php endif; ?>        
			<?php if ($page != $last_page) : ?>     
				<a href="#" class="next"><span><?php _e('Next &raquo;', 'review'); ?></span></a>      
			<?php endif; ?>          
		</div>                   
		<div class="clear"></div>
