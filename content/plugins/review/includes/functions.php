...
/* Show with shortcode */
add_shortcode('review', 'wp_review_shortcode');
add_shortcode('review-total', 'wp_review_total_shortcode');
// aliases
...
function wp_review_get_data( $post_id = null ) {
	...
	// using this second if() instead of the first will allow reviews to be displayed on archive pages, but it may mess up excerpts
			$review = '<div id="review" class="review-wrapper review-'.$post_id.' ' . $class . ' delay-animation" >';
			...
			/**
				...
				*/
				...
				if( is_array( $allowUsers ) && $allowUsers[0] == 1 && $post_id == $post->ID ){								
					...
					$review .= '<div class="user-review-area" title="'.__('Click on the stars to rate!', 'review').'">';
					...
					$review .= '<input type="hidden" id="token" value="'.wp_create_nonce( "review-security" ).'">';	
                    
					
					if( $userTotal == '' ) $userTotal = '0.0';
					$review .= '<div class="user-total-wrapper"><span class="user-review-title">'.__('User Rating','review').': </span><span class="review-total-box"><span id="mts-user-reviews-total">' . $userTotal . '</span> ';
					$review.= '<small>(<span id="mts-user-reviews-counter" >'.$usersReviewsCount.'</span> '.__('votes', 'review').')</small></span></div>';
					...
					$review .= '<div class="review-total-star '.$allowedClass.' '.$hasNotRatedClass.'" >';
						$review .='<div class="mts-review-wait-msg"><span class="animate-spin mts-icon-loader"></span>'.__('Sending','review').'</div>';
						$review .= '<div class="review-result-wrapper">';
...
function wp_review_show_total($echo = true, $class = 'review-total-only', $post_id = null) {
    ...
    if (!empty($type) && (!empty($total) || $total === '0')) {
        wp_enqueue_style( 'wp_review-style', trailingslashit( WP_REVIEW_ASSETS ) . 'css/review.css', array(), WP_REVIEW_PLUGIN_VERSION, 'all' );

        $review = '<div class="review-type-'.$type.' '.esc_attr($class).' review-show-total review-total-'.$post_id.'"> ';
    	
    	if ( 'percentage' == $type ) {
    		$review .= '<span class="review-total-box"><span>' . $total . '</span> <i class="percentage-icon">%</i>' . '</span>';
    	} elseif ( 'point' == $type ) {
    		$review .= '<span class="review-total-box">' . $total . '/'.__('10','review').'</span>';
    	} elseif ( 'star' == $type ) {
    	    // star
    		$review .= '<div class="review-total-star">';
    			$review .= '<div class="review-result-wrapper">';
...
function wp_review_color_output($post_id) {
	...
	if(is_singular()) {
		...
		if ( 'star' == $type ) {
			...
			$style .= '.review-'.$post_id.' .review-result-wrapper .review-result i { color: '.$color.'; opacity: 1; filter: alpha(opacity=100); }
				.review-'.$post_id.' .review-result-wrapper i{ color: '.$color.'; opacity: 0.50; filter: alpha(opacity=50); }';
				
		} elseif ( 'point' == $type ) {

			$style .= '.review-'.$post_id.'.bar-point .review-result { background-color: '.$color.'; }';

		} elseif ( 'percentage' == $type ) { 

			$style .= '.review-'.$post_id.'.percentage-point .review-result { background-color: '.$color.'; }';

		}

		$style .= '.review-'.$post_id.'.review-wrapper, .review-'.$post_id.' .review-title, .review-'.$post_id.' .review-desc p{ color: '.$fontcolor.';}
		.review-'.$post_id.' .review-list li, .review-'.$post_id.'.review-wrapper{ background: '.$bgcolor2.';}
		.review-'.$post_id.' .review-title, .review-'.$post_id.' .review-list li:nth-child(2n){background: '.$bgcolor1.';}

		.review-'.$post_id.'.bar-point .allowed-to-rate .review-result, .review-'.$post_id.'.percentage-point .allowed-to-rate .review-result{background: none;}
		.review-'.$post_id.' .review-total-star.allowed-to-rate a i, .review-comment-field a i, .review-comment-rating a i { color: '.$color.'; opacity: 0.50; filter: alpha(opacity=50); }
		.review-'.$post_id.'.bar-point .allowed-to-rate .review-result, .review-'.$post_id.'.percentage-point .allowed-to-rate .review-result{text-indent:0;}
		.review-'.$post_id.'.bar-point .allowed-to-rate .review-result i, .review-'.$post_id.'.percentage-point .allowed-to-rate .review-result i, .review-'.$post_id.' .mts-user-review-star-container .selected i, .review-'.$post_id.' .user-review-area .review-result i, .review-comment-field .review-result i, .review-comment-rating .review-result i { color: '.$color.'; opacity: 1; filter: alpha(opacity=100); }
		.review-'.$post_id.'.review-wrapper, .review-'.$post_id.' .review-title, .review-'.$post_id.' .review-list li, .review-'.$post_id.' .review-list li:last-child, .review-'.$post_id.' .user-review-area{border-color: '.$bordercolor.';}';
		
		if( $total == '' ){

		$style .= '.review-'.$post_id.' .user-review-area{border: 1px solid '.$bordercolor.'; margin-top: 0px;}
			.review-'.$post_id.' .review-desc{width: 100%;}
			.review-'.$post_id.'.review-wrapper{border: none; overflow: visible;}';
		}
		$style .= '</style>';
...
function mts_review_get_review(){
    // security
    check_ajax_referer( 'review-security', 'nonce' );
