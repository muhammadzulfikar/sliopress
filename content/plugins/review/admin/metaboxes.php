...
function wp_review_add_meta_boxes() {
    ...
    foreach ($post_types as $post_type) {
        if (!in_array($post_type, $excluded_post_types)) {
        	...
        	add_meta_box(
        		'review-metabox-heading',
        		__( 'Review Heading', 'review' ),
        	...
        	add_meta_box(
        		'review-metabox-userReview',
        		__( 'User Reviews', 'review' ),
...
function wp_review_render_meta_box_review_options( $post ) {
	...
	wp_nonce_field( basename( __FILE__ ), 'review-review-options-nonce' );

	/* Retrieve an existing value from the database. */
	$type = get_post_meta( $post->ID, 'wp_review_type', true );
    
    $available_types = apply_filters('wp_review_metabox_types', array('star' => __('Star', 'review'), 'point' => __('Point', 'review'), 'percentage' => __('Percentage', 'review')));
?>
	
	<p class="review-field">
		<label for="wp_review_type"><?php _e( 'Review Type', 'review' ); ?></label>
		<select name="wp_review_type" id="wp_review_type">
			<option value=""><?php _e( 'No Review', 'review' ) ?></option>
...
function wp_review_render_meta_box_item( $post ) {
	...
	/* Add an nonce field so we can check for it later. */
	wp_nonce_field( basename( __FILE__ ), 'review-item-nonce' ); 
...
?>
	...
	<table id="review-item" class="review-item" width="100%">

		<thead>
			<tr>
				<th width="80%"><?php _e( 'Feature Name', 'review' ); ?></th>
				<th width="10%" class="dynamic-text"><?php _e( 'Star (1-5)', 'review' ); ?></th>
				<th width="10%"></th>
			</tr>
		</thead>
			...
			<?php if ( !empty($items) ) : ?>
				...
				<?php foreach ( $items as $item ) { ?>
					...
					<tr>
						...
						<td><a class="button remove-row" href="#"><?php _e( 'Delete', 'review' ); ?></a></td>
					</tr>
	...
	<table width="100%">
		<tr>
			<td width="80%"><a id="add-row" class="button" href="#"><?php _e( 'Add another', 'review' ) ?></a></td>
			<td width="10%">
				<input type="text" class="widefat review-total" name="wp_review_total" value="<?php echo get_post_meta( $post->ID, 'wp_review_total', true ); ?>" />
			</td>
			<td width="10%"><?php _e( 'Total', 'review' ); ?></td>
		</tr>
	</table>

	<p class="review-field">
		<label for="wp_review_custom_location"><?php _e( 'Custom Location', 'review' ); ?></label>
    ...
    <div class="review-location-options"<?php if (empty($custom_location)) echo ' style="display: none;"'; ?>>
		<p class="review-field">
			<label for="wp_review_location"><?php _e( 'Review Location', 'review' ); ?></label>
			<select name="wp_review_location" id="wp_review_location">
				<option value="bottom" <?php selected( $location, 'bottom' ); ?>><?php _e( 'After Content', 'review' ) ?></option>
				<option value="top" <?php selected( $location, 'top' ); ?>><?php _e( 'Before Content', 'review' ) ?></option>
	            <option value="custom" <?php selected( $location, 'custom' ); ?>><?php _e( 'Custom (use shortcode)', 'review' ) ?></option>
			</select>
		</p>
		<p class="review-field" id="wp_review_shortcode_hint_field">
			<label for="wp_review_shortcode_hint"></label>
			<input id="wp_review_shortcode_hint" type="text" value="[review]" readonly="readonly" />
	        <span><?php _e('Copy &amp; paste this shortcode in the content.', 'review') ?></span>
		</p>
	</div>
	<p class="review-field">
		<label for="wp_review_custom_colors"><?php _e( 'Custom Colors', 'review' ); ?></label>
    ...
    <div class="review-color-options"<?php if (empty($custom_colors)) echo ' style="display: none;"'; ?>>

		<p class="review-field"<?php if (empty($displayed_fields['color'])) echo ' style="display: none;"'; ?>>
			<label for="wp_review_color"><?php _e( 'Review Color', 'review' ); ?></label>
			<input type="text" class="review-color" name="wp_review_color" value="<?php echo $color; ?>" />
		</p>

		<p class="review-field"<?php if (empty($displayed_fields['fontcolor'])) echo ' style="display: none;"'; ?>>
			<label for="wp_review_fontcolor"><?php _e( 'Font Color', 'review' ); ?></label>
			<input type="text" class="review-color" name="wp_review_fontcolor" id ="wp_review_fontcolor" value="<?php echo $fontcolor; ?>" />
		</p>

		<p class="review-field"<?php if (empty($displayed_fields['bgcolor1'])) echo ' style="display: none;"'; ?>>
			<label for="wp_review_bgcolor1"><?php _e( 'Heading Background Color', 'review' ); ?></label>
			<input type="text" class="review-color" name="wp_review_bgcolor1" id ="wp_review_bgcolor1" value="<?php echo $bgcolor1; ?>" />
		</p>

		<p class="review-field"<?php if (empty($displayed_fields['bgcolor2'])) echo ' style="display: none;"'; ?>>
			<label for="wp_review_bgcolor2"><?php _e( 'Background Color', 'review' ); ?></label>
			<input type="text" class="review-color" name="wp_review_bgcolor2" id="wp_review_bgcolor2" value="<?php echo $bgcolor2; ?>" />
		</p>

		<p class="review-field"<?php if (empty($displayed_fields['bordercolor'])) echo ' style="display: none;"'; ?>>
			<label for="wp_review_bordercolor"><?php _e( 'Border Color', 'review' ); ?></label>
			<input type="text" class="review-color" name="wp_review_bordercolor" id="wp_review_bordercolor" value="<?php echo $bordercolor; ?>" />
		</p>
	</div>
...
function wp_review_render_meta_box_heading( $post ) {
	/* Add an nonce field so we can check for it later. */
	wp_nonce_field( basename( __FILE__ ), 'review-heading-nonce' ); 
	
	/* Retrieve an existing value from the database. */
	$heading = get_post_meta( $post->ID, 'wp_review_heading', true );	
	?>
	<p class="review-field">
		<label><?php _e( 'Review Heading', 'review' ); ?></label>
...
function wp_review_render_meta_box_desc( $post ) {
	...
	/* Add an nonce field so we can check for it later. */
	wp_nonce_field( basename( __FILE__ ), 'review-desc-nonce' ); 
	...
	if (!$desc_title) $desc_title = __('Summary', 'review');
	?>
	<p id="review-desc-title" class="review-field">
	...
	<p class="review-field">
		<label style="width: 100%;">
			...
			<?php _e( 'Hide Description &amp; Total Rating', 'review' ); ?>
		</label>
	</p>
	<?php
}
...
function wp_review_render_meta_box_userReview( $post ) {
	/* Add an nonce field so we can check for it later. */
	wp_nonce_field( basename( __FILE__ ), 'review-userReview-nonce' ); 
	...
	<p class="review-field">
		<label for="review-userReview-disable"> <?php _e( 'Disabled', 'review' ); ?>
			<input type="radio" name="wp_review_userReview[]" id="review-userReview-disable" class="review-userReview-options" value="0" <?php echo !$enabled ? 'checked' : ''; ?> />
		</label>
		
		<label for="review-userReview-enable"> <?php _e( 'Enabled', 'review' ); ?>
			<input type="radio" name="wp_review_userReview[]" id="review-userReview-enable" class="review-userReview-options" value="1" <?php echo $enabled ? 'checked' : ''; ?> />
		</label>
	</p>
...
function wp_review_save_postdata( $post_id, $post ) {
	...
	if ( !isset( $_POST['review-review-options-nonce'] ) || !wp_verify_nonce( $_POST['review-review-options-nonce'], basename( __FILE__ ) ) )
		return;

	if ( !isset( $_POST['review-item-nonce'] ) || !wp_verify_nonce( $_POST['review-item-nonce'], basename( __FILE__ ) ) )
		return;

	if ( !isset( $_POST['review-heading-nonce'] ) || !wp_verify_nonce( $_POST['review-heading-nonce'], basename( __FILE__ ) ) )
		return;
	
	if ( !isset( $_POST['review-desc-nonce'] ) || !wp_verify_nonce( $_POST['review-desc-nonce'], basename( __FILE__ ) ) )
		return;
	
	if ( !isset( $_POST['review-userReview-nonce'] ) || !wp_verify_nonce( $_POST['review-userReview-nonce'], basename( __FILE__ ) ) )
		return;
