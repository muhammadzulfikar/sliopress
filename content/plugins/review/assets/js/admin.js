...
* Plugin URI: http://mythemeshop.com/plugins/review/
*/
...
jQuery(document).ready(function($) {
	...
	$('#add-row').on('click', function(e) {
		...
		row.insertBefore('#review-item tbody>tr:last');
	 ...
	 * Toggle meta box
	 */
	$('#review-metabox-item').hide();
	$('#review-metabox-heading').hide();
	$('#review-metabox-desc').hide();
	$('#review-metabox-userReview').hide();
    $('#wp_review_shortcode_hint_field').hide();
    $('#wp_review_id_hint').hide();
    
	$('#wp_review_type').on( 'change', function() {

		$('#review-metabox-item').toggle( $(this).val() != '' );
		$('#review-metabox-heading').toggle( $(this).val() != '' );
		$('#review-metabox-desc').toggle( $(this).val() != '' );
		$('#review-metabox-userReview').toggle( $(this).val() != '' );	
	...
	$('#wp_review_custom_colors').change(function(e) {
		if ( $(this).is(':checked') ) {
			$('.review-color-options').show();
		} else {
			$('.review-color-options').hide();
		}
	});
	$('#wp_review_custom_location').change(function(e) {
		if ( $(this).is(':checked') ) {
			$('.review-location-options').show();
		} else {
			$('.review-location-options').hide();
		}
	});
	$('#wp_review_custom_width').change(function(e) {
		if ( $(this).is(':checked') ) {
			$('.review-width-options').show();
		} else {
			$('.review-width-options').hide();
		}
	});

	$('#wp_review_desc-wrap').toggle(! $('#wp_review_hide_desc').is(':checked'));
	$('#wp_review_hide_desc').change(function() {
		$('#wp_review_desc-wrap, #review-desc-title').toggle(!$(this).is(':checked'));
	});

	if ( $('#wp_review_type option:selected').val() === 'star' ) {
		$('#review-metabox-item').show();
		$('#review-metabox-heading').show();
		$('#review-metabox-desc').show();
		$('#review-metabox-userReview').show();
	}
	...
	$.extend({
		...
		review_total: function(){
			$('.review-star').on( 'change', function () {
				...
				$('.review-total').val( Math.round(sum * 10) / 10 );
	 ...
	 * Color picker setup
	 */
	$('.review-color').wpColorPicker();

	$('.review-theme-defaults-msg .close-notice').click(function() {
		$('.review-theme-defaults-msg').remove();
	});
	$('.review-theme-defaults-msg a.button').click(function() {
