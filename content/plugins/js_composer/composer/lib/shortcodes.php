    ...
    abstract class WPBakeryShortCode extends WPBakeryVisualComposerAbstract {
        ...
        protected function loadTemplate( $atts, $content = null) {
            ...
            if($this->html_template) {
                ...
                trigger_error(sprintf(__('Template file is missing for `%s` shortcode. Make sure you have `%s` file in your theme folder.', 'js_composer'), $this->shortcode, 'content/themes/your_theme/vc_templates/'.$this->shortcode.'.php'));
            }
            return $output;
        }
...
if (!class_exists('WPBakeryShortCode_UniversalAdmin')) {
    abstract class WPBakeryShortCode_UniversalAdmin extends WPBakeryShortCode {
        ...
        protected function getTinyHtmlTextArea($param = array(), $param_value) {
            ...
            //$upload_media_btns = '<div class="wpb_media-buttons hide-if-no-js"> '.__('Upload/Insert').' <a title="'.__('Add an Image').'" class="wpb_insert-image" href="#"><img alt="'.__('Add an Image').'" src="'.home_url().'/admin/images/media-button-image.gif"></a> <a class="wpb_switch-editors" title="'.__('Switch Editors').'" href="#">HTML mode</a></div>';
