...
!function($) {
    ...
    function init_textarea_html($element) {
         ...
         */
        if($('#link').parent().hasClass('dialog')) $('#link').wpdialog('destroy');
    ...
    function init_textarea_html_old($element) {
        ...
        $form_line.find('.switch-editor').removeAttr("onclick");
         $('.switch-tmce').trigger('click');
         $element.closest('.edit_form_line').find('.switch-tmce').click(function () {
         window.tinyMCE.execCommand("mceAddControl", true, textfield_id);
         window.switchEditors.go(textfield_id, 'tmce');
         $element.closest('.edit_form_line').find('.editor-wrap').removeClass('html-active').addClass('tmce-active');
         ...
         $element.closest('.edit_form_line').find('.switch-html').click(function () {
             $element.closest('.edit_form_line').find('.editor-wrap').removeClass('tmce-active').addClass('html-active');
  ...
  $('.vc-color-control').length && $.fn.wpColorPicker && $('.vc-color-control').wpColorPicker();
    ...
    $('.vc-link-build').click(function(e){
        ...
        $vc_link_submit.unbind('click.vcLink').bind('click.vcLink', function(e){
            ...
            $('#link-cancel').unbind('click.vcLink');
            window.wpLink.textarea = '';
            return false;
        });
        $('#link-cancel').unbind('click.vcLink').bind('click.vcLink', function(e){
            e.preventDefault();
            $dialog.wpdialog('close');
            // remove vc_link hooks for wpLink
            $vc_link_submit.unbind('click.vcLink');
