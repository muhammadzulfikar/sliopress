...
|	File Information:											|
|	- Manage Your Polls											|
|	- content/plugins/polls/polls-manager.php				|
|																|
+----------------------------------------------------------------+
*/
...
if(!empty($_POST['do'])) {
	...
	switch($_POST['do']) {
		// Edit Poll
		case __('Edit Poll', 'polls'):
			check_admin_referer('polls_edit-poll');
			// Poll ID
			...
			if(!$edit_poll_question) {
				$text = '<p style="color: blue">'.sprintf(__('No Changes Had Been Made To Poll\'s Question \'%s\'.', 'polls'), stripslashes($pollq_question)).'</p>';
			}
			...
			if($get_polla_aids) {
				...
				foreach($polla_aids as $polla_aid) {
					...
					if(!$edit_poll_answer) {
						$text .= '<p style="color: blue">'.sprintf(__('No Changes Had Been Made To Poll\'s Answer \'%s\'.', 'polls'), stripslashes($polla_answers)).'</p>';
					} else {
						$text .= '<p style="color: green">'.sprintf(__('Poll\'s Answer \'%s\' Edited Successfully.', 'polls'), stripslashes($polla_answers)).'</p>';
					}
				}
			} else {
				$text .= '<p style="color: red">'.sprintf(__('Invalid Poll \'%s\'.', 'polls'), stripslashes($pollq_question)).'</p>';
			}
			...
			if(!empty($polla_answers_new)) {
				...
				foreach($polla_answers_new as $polla_answer_new) {
					...
					if(!empty($polla_answer_new)) {
						...
						if(!$add_poll_answers) {
							$text .= '<p style="color: red;">'.sprintf(__('Error In Adding Poll\'s Answer \'%s\'.', 'polls'), stripslashes($polla_answer_new)).'</p>';
						} else {
							$text .= '<p style="color: green;">'.sprintf(__('Poll\'s Answer \'%s\' Added Successfully.', 'polls'), stripslashes($polla_answer_new)).'</p>';
						}
					}
					$i++;
				}
			...
			if(empty($text)) {
				$text = '<p style="color: green">'.sprintf(__('Poll \'%s\' Edited Successfully.', 'polls'), stripslashes($pollq_question)).'</p>';
			}
...
?>
		...
		<?php wp_nonce_field('polls_edit-poll'); ?>
		<input type="hidden" name="pollq_id" value="<?php echo $poll_id; ?>" />
		<input type="hidden" name="pollq_active" value="<?php echo $poll_active; ?>" />
		<div class="wrap">
			<h2><?php _e('Edit Poll', 'polls'); ?></h2>
			<!-- Poll Question -->
			<h3><?php _e('Poll Question', 'polls'); ?></h3>
			<table class="form-table">
				<tr>
					<th width="20%" scope="row" valign="top"><?php _e('Question', 'polls') ?></th>
					<td width="80%"><input type="text" size="70" name="pollq_question" value="<?php echo htmlspecialchars($poll_question_text); ?>" /></td>
				</tr>
			</table>
			<!-- Poll Answers -->
			<h3><?php _e('Poll Answers', 'polls'); ?></h3>
			<table class="form-table">
				<thead>
					<tr>
						<th width="20%" scope="row" valign="top"><?php _e('Answer No.', 'polls') ?></th>
						<th width="60%" scope="row" valign="top"><?php _e('Answer Text', 'polls') ?></th>
						<th width="20%" scope="row" valign="top" style="text-align: <?php echo $last_col_align; ?>;"><?php _e('No. Of Votes', 'polls') ?></th>
					</tr>
				</thead>
				<tbody id="poll_answers">
					<?php
						...
						if($poll_answers) {
							...
							$pollip_answers[0] = __('Null Votes', 'polls');
							foreach($poll_answers as $poll_answer) {
								...
								echo '<th width="20%" scope="row" valign="top">'.sprintf(__('Answer %s', 'polls'), number_format_i18n($i)).'</th>'."\n";
								echo "<td width=\"60%\"><input type=\"text\" size=\"50\" maxlength=\"200\" name=\"polla_aid-$polla_aid\" value=\"".htmlspecialchars($polla_answers)."\" />&nbsp;&nbsp;&nbsp;";
								echo "<input type=\"button\" value=\"".__('Delete', 'polls')."\" onclick=\"delete_poll_ans($poll_id, $polla_aid, $polla_votes, '".sprintf(esc_js(__('You are about to delete this poll\'s answer \'%s\'.', 'polls')), esc_js(htmlspecialchars($polla_answers)))."', '".wp_create_nonce('polls_delete-poll-answer')."');\" class=\"button\" /></td>\n";
				...
				<tbody>
					<tr>
						<td width="20%">&nbsp;</td>
						<td width="60%"><input type="button" value="<?php _e('Add Answer', 'polls') ?>" onclick="add_poll_answer_edit();" class="button" /></td>
						<td width="20%" align="<?php echo $last_col_align; ?>"><strong><?php _e('Total Votes:', 'polls'); ?></strong> <strong id="poll_total_votes"><?php echo number_format_i18n($poll_actual_totalvotes); ?></strong> <input type="text" size="4" readonly="readonly" id="pollq_totalvotes" name="pollq_totalvotes" value="<?php echo $poll_actual_totalvotes; ?>" onblur="check_totalvotes();" /></td>
					</tr>
					<tr>
						<td width="20%">&nbsp;</td>
						<td width="60%">&nbsp;</td>
						<td width="20%" align="<?php echo $last_col_align; ?>"><strong><?php _e('Total Voters:', 'polls'); ?> <?php echo number_format_i18n($poll_totalvoters); ?></strong> <input type="text" size="4" name="pollq_totalvoters" value="<?php echo $poll_totalvoters; ?>" /></td>
					</tr>
				</tbody>
			</table>
			<!-- Poll Multiple Answers -->
			<h3><?php _e('Poll Multiple Answers', 'polls') ?></h3>
			<table class="form-table">
				<tr>
					<th width="40%" scope="row" valign="top"><?php _e('Allows Users To Select More Than One Answer?', 'polls'); ?></th>
					<td width="60%">
						<select name="pollq_multiple_yes" id="pollq_multiple_yes" size="1" onchange="check_pollq_multiple();">
							<option value="0"<?php selected('0', $poll_multiple); ?>><?php _e('No', 'polls'); ?></option>
							<option value="1"<?php if($poll_multiple > 0) { echo ' selected="selected"'; } ?>><?php _e('Yes', 'polls'); ?></option>
						</select>
					</td>
				</tr>
				<tr>
					<th width="40%" scope="row" valign="top"><?php _e('Maximum Number Of Selected Answers Allowed?', 'polls') ?></th>
					<td width="60%">
			...
			<!-- Poll Start/End Date -->
			<h3><?php _e('Poll Start/End Date', 'polls'); ?></h3>
			<table class="form-table">
				<tr>
					<th width="20%" scope="row" valign="top"><?php _e('Start Date/Time', 'polls'); ?></th>
					<td width="80%">
						<?php echo mysql2date(sprintf(__('%s @ %s', 'polls'), get_option('date_format'), get_option('time_format')), gmdate('Y-m-d H:i:s', $poll_timestamp)); ?><br />
						<input type="checkbox" name="edit_polltimestamp" id="edit_polltimestamp" value="1" onclick="check_polltimestamp()" />&nbsp;<label for="edit_polltimestamp"><?php _e('Edit Start Date/Time', 'polls'); ?></label><br />
						<?php poll_timestamp($poll_timestamp, 'pollq_timestamp', 'none'); ?>
					</td>
				</tr>
					<tr>
					<th width="20%" scope="row" valign="top"><?php _e('End Date/Time', 'polls'); ?></th>
					<td width="80%">
						<?php
							if(empty($poll_expiry)) {
								_e('This Poll Will Not Expire', 'polls');
							} else {
								echo mysql2date(sprintf(__('%s @ %s', 'polls'), get_option('date_format'), get_option('time_format')), gmdate('Y-m-d H:i:s', $poll_expiry));
							}
						?>
						<br />
						...
						<label for="pollq_expiry_no"><?php _e('Do NOT Expire This Poll', 'polls'); ?></label><br />
						<?php
							if(empty($poll_expiry)) {
			...
			<p style="text-align: center;">
				<input type="submit" name="do" value="<?php _e('Edit Poll', 'polls'); ?>" class="button-primary" />&nbsp;&nbsp;
			<?php
				if($poll_active == 1) {
			...
			?>
				<input type="button" class="button" name="do" id="close_poll" value="<?php _e('Close Poll', 'polls'); ?>" onclick="closing_poll(<?php echo $poll_id; ?>, '<?php printf(esc_js(__('You are about to CLOSE this poll \'%s\'.', 'polls')), htmlspecialchars(esc_js($poll_question_text))); ?>', '<?php echo wp_create_nonce('polls_close-poll'); ?>');" style="display: <?php echo $poll_close_display; ?>;" />
				<input type="button" class="button" name="do" id="open_poll" value="<?php _e('Open Poll', 'polls'); ?>" onclick="opening_poll(<?php echo $poll_id; ?>, '<?php printf(esc_js(__('You are about to OPEN this poll \'%s\'.', 'polls')), htmlspecialchars(esc_js($poll_question_text))); ?>', '<?php echo wp_create_nonce('polls_open-poll'); ?>');" style="display: <?php echo $poll_open_display; ?>;" />
				&nbsp;&nbsp;<input type="button" name="cancel" value="<?php _e('Cancel', 'polls'); ?>" class="button" onclick="javascript:history.go(-1)" />
			</p>
		</div>
		</form>
<?php
...
?>
		...
		<!-- Manage Polls -->
		<div class="wrap">
			<h2><?php _e('Manage Polls', 'polls'); ?></h2>
			<h3><?php _e('Polls', 'polls'); ?></h3>
			<br style="clear" />
			<table class="widefat">
				<thead>
					<tr>
						<th><?php _e('ID', 'polls'); ?></th>
						<th><?php _e('Question', 'polls'); ?></th>
						<th><?php _e('Total Voters', 'polls'); ?></th>
						<th><?php _e('Start Date/Time', 'polls'); ?></th>
						<th><?php _e('End Date/Time', 'polls'); ?></th>
						<th><?php _e('Status', 'polls'); ?></th>
						<th colspan="3"><?php _e('Action', 'polls'); ?></th>
					</tr>
				</thead>
					...
					<?php
						if($polls) {
							...
							foreach($polls as $poll) {
								...
								$poll_date = mysql2date(sprintf(__('%s @ %s', 'polls'), get_option('date_format'), get_option('time_format')), gmdate('Y-m-d H:i:s', $poll->pollq_timestamp));
								...
								if(empty($poll_expiry)) {
									$poll_expiry_text  = __('No Expiry', 'polls');
								} else {
									$poll_expiry_text = mysql2date(sprintf(__('%s @ %s', 'polls'), get_option('date_format'), get_option('time_format')), gmdate('Y-m-d H:i:s', $poll_expiry));
								}
								if($i%2 == 0) {
								...
								if($current_poll > 0) {
									if($current_poll == $poll_id) {
										echo '<strong>'.__('Displayed:', 'polls').'</strong> ';
									}
								} elseif($current_poll == 0) {
									if($poll_id == $latest_poll) {
										echo '<strong>'.__('Displayed:', 'polls').'</strong> ';
								...
								echo "</td>\n";
								echo "<td><a href=\"$base_page&amp;mode=logs&amp;id=$poll_id\" class=\"edit\">".__('Logs', 'polls')."</a></td>\n";
								echo "<td><a href=\"$base_page&amp;mode=edit&amp;id=$poll_id\" class=\"edit\">".__('Edit', 'polls')."</a></td>\n";
								echo "<td><a href=\"#DeletePoll\" onclick=\"delete_poll($poll_id, '".sprintf(esc_js(__('You are about to delete this poll, \'%s\'.', 'polls')), esc_js($poll_question))."', '".wp_create_nonce('polls_delete-poll')."');\" class=\"delete\">".__('Delete', 'polls')."</a></td>\n";
								echo '</tr>';
								$i++;
							...
							echo '<tr><td colspan="9" align="center"><strong>'.__('No Polls Found', 'polls').'</strong></td></tr>';
						}
					?>
				</tbody>
			</table>
		...
		<!-- Polls Stats -->
		<div class="wrap">
			<h3><?php _e('Polls Stats:', 'polls'); ?></h3>
			<br style="clear" />
			<table class="widefat">
			<tr>
				<th><?php _e('Total Polls:', 'polls'); ?></th>
				<td><?php echo number_format_i18n($i); ?></td>
			</tr>
			<tr class="alternate">
				<th><?php _e('Total Polls\' Answers:', 'polls'); ?></th>
				<td><?php echo number_format_i18n($total_ans); ?></td>
			</tr>
			<tr>
				<th><?php _e('Total Votes Cast:', 'polls'); ?></th>
		...
		<div class="wrap">
			...
			<?php
				...
				if($poll_ips > 0) {
			?>
				<strong><?php _e('Are You Sure You Want To Delete All Polls Logs?', 'polls'); ?></strong><br /><br />
				<input type="checkbox" name="delete_logs_yes" id="delete_logs_yes" value="yes" />&nbsp;<label for="delete_logs_yes"><?php _e('Yes', 'polls'); ?></label><br /><br />
				<input type="button" value="<?php _e('Delete All Logs', 'polls'); ?>" class="button" onclick="delete_poll_logs('<?php echo esc_js(__('You are about to delete all poll logs. This action is not reversible.', 'polls')); ?>', '<?php echo wp_create_nonce('polls_delete-polls-logs'); ?>');" />
			<?php
				} else {
					_e('No poll logs available.', 'polls');
				}
			?>
			</div>
			<p><?php _e('Note: If your logging method is by IP and Cookie or by Cookie, users may still be unable to vote if they have voted before as the cookie is still stored in their computer.', 'polls'); ?></p>
		</div>
<?php
} // End switch($mode)
