...
function polls_textdomain() {
	load_plugin_textdomain( 'polls', false, dirname( plugin_basename( __FILE__ ) ) );
}
...
function poll_menu() {
	add_menu_page( __( 'Polls', 'polls' ), __( 'Polls', 'polls' ), 'manage_polls', 'polls/polls-manager.php', '', 'dashicons-chart-bar' );

	add_submenu_page( 'polls/polls-manager.php', __( 'Manage Polls', 'polls'), __( 'Manage Polls', 'polls' ), 'manage_polls', 'polls/polls-manager.php' );
	add_submenu_page( 'polls/polls-manager.php', __( 'Add Poll', 'polls'), __( 'Add Poll', 'polls' ), 'manage_polls', 'polls/polls-add.php' );
	add_submenu_page( 'polls/polls-manager.php', __( 'Poll Options', 'polls'), __( 'Poll Options', 'polls' ), 'manage_polls', 'polls/polls-options.php' );
	add_submenu_page( 'polls/polls-manager.php', __( 'Poll Templates', 'polls'), __( 'Poll Templates', 'polls' ), 'manage_polls', 'polls/polls-templates.php' );
}
...
function poll_head_scripts() {
	...
	if($pollbar['style'] == 'use_css') {
		...
		echo "\t".'line-height: '.$pollbar['height'].'px;'."\n";
		echo "\t".'height: '.$pollbar['height'].'px;'."\n";
		echo "\t".'background-image: url(\''.plugins_url('polls/images/'.$pollbar['style'].'/pollbg.gif').'\');'."\n";
		echo "\t".'border: 1px solid #'.$pollbar['border'].';'."\n";
		echo '}'."\n";
	}
	echo '</style>'."\n";
...
function poll_scripts() {
	if(@file_exists(get_stylesheet_directory().'/polls-css.css')) {
		wp_enqueue_style('polls', get_stylesheet_directory_uri().'/polls-css.css', false, WP_POLLS_VERSION, 'all');
	} else {
		wp_enqueue_style('polls', plugins_url('polls/polls-css.css'), false, WP_POLLS_VERSION, 'all');
	}
	if( is_rtl() ) {
		if(@file_exists(get_stylesheet_directory().'/polls-css-rtl.css')) {
			wp_enqueue_style('polls-rtl', get_stylesheet_directory_uri().'/polls-css-rtl.css', false, WP_POLLS_VERSION, 'all');
		} else {
			wp_enqueue_style('polls-rtl', plugins_url('polls/polls-css-rtl.css'), false, WP_POLLS_VERSION, 'all');
		}
	}
	$poll_ajax_style = get_option('poll_ajax_style');
	$pollbar = get_option('poll_bar');
	wp_enqueue_script('polls', plugins_url('polls/polls-js.js'), array('jquery'), WP_POLLS_VERSION, true);
	wp_localize_script('polls', 'pollsL10n', array(
		'ajax_url' => admin_url('admin-ajax.php'),
		'text_wait' => __('Your last request is still being processed. Please wait a while ...', 'polls'),
		'text_valid' => __('Please choose a valid poll answer.', 'polls'),
		'text_multiple' => __('Maximum number of choices allowed: ', 'polls'),
...
### Function: Enqueue Polls Stylesheets/JavaScripts In Admin
add_action('admin_enqueue_scripts', 'poll_scripts_admin');
function poll_scripts_admin($hook_suffix) {
	$poll_admin_pages = array('polls/polls-manager.php', 'polls/polls-add.php', 'polls/polls-options.php', 'polls/polls-templates.php', 'polls/polls-uninstall.php');
	if(in_array($hook_suffix, $poll_admin_pages)) {
		wp_enqueue_style('polls-admin', plugins_url('polls/polls-admin-css.css'), false, WP_POLLS_VERSION, 'all');
		wp_enqueue_script('polls-admin', plugins_url('polls/polls-admin-js.js'), array('jquery'), WP_POLLS_VERSION, true);
		wp_localize_script('polls-admin', 'pollsAdminL10n', array(
			'admin_ajax_url' => admin_url('admin-ajax.php'),
			'text_direction' => is_rtl() ? 'right' : 'left',
			'text_delete_poll' => __('Delete Poll', 'polls'),
			'text_no_poll_logs' => __('No poll logs available.', 'polls'),
			'text_delete_all_logs' => __('Delete All Logs', 'polls'),
			'text_checkbox_delete_all_logs' => __('Please check the \\\'Yes\\\' checkbox if you want to delete all logs.', 'polls'),
			'text_delete_poll_logs' => __('Delete Logs For This Poll Only', 'polls'),
			'text_checkbox_delete_poll_logs' => __('Please check the \\\'Yes\\\' checkbox if you want to delete all logs for this poll ONLY.', 'polls'),
			'text_delete_poll_ans' => __('Delete Poll Answer', 'polls'),
			'text_open_poll' => __('Open Poll', 'polls'),
			'text_close_poll' => __('Close Poll', 'polls'),
			'text_answer' => __('Answer', 'polls'),
			'text_remove_poll_answer' => __('Remove', 'polls')
		));
	}
}


### Function: Displays Polls Footer In Admin
...
?>
	<script type="text/javascript">
		QTags.addButton('ed_wp_polls', '<?php echo esc_js(__('Poll', 'polls')); ?>', function() {
			var poll_id = jQuery.trim(prompt('<?php echo esc_js(__('Enter Poll ID', 'polls')); ?>'));
			while(isNaN(poll_id)) {
				poll_id = jQuery.trim(prompt("<?php echo esc_js(__('Error: Poll ID must be numeric', 'polls')); ?>\n\n<?php echo esc_js(__('Please enter Poll ID again', 'polls')); ?>"));
			}
...
function poll_tinymce_addplugin($plugin_array) {
	if(WP_DEBUG) {
		$plugin_array['polls'] = plugins_url( 'polls/tinymce/plugins/polls/plugin.js?v=' . WP_POLLS_VERSION );
	} else {
		$plugin_array['polls'] = plugins_url( 'polls/tinymce/plugins/polls/plugin.min.js?v=' . WP_POLLS_VERSION );
	}
	return $plugin_array;
}
function poll_tinymce_translation($mce_translation) {
	$mce_translation['Enter Poll ID'] = esc_js(__('Enter Poll ID', 'polls'));
	$mce_translation['Error: Poll ID must be numeric'] = esc_js(__('Error: Poll ID must be numeric', 'polls'));
	$mce_translation['Please enter Poll ID again'] = esc_js(__('Please enter Poll ID again', 'polls'));
	$mce_translation['Insert Poll'] = esc_js(__('Insert Poll', 'polls'));
...
function display_pollvote($poll_id, $display_loading = true) {
	...
	$poll_start_date = mysql2date(sprintf(__('%s @ %s', 'polls'), get_option('date_format'), get_option('time_format')), gmdate('Y-m-d H:i:s', $poll_question->pollq_timestamp));
	$poll_expiry = trim($poll_question->pollq_expiry);
	if(empty($poll_expiry)) {
		$poll_end_date  = __('No Expiry', 'polls');
	} else {
		$poll_end_date  = mysql2date(sprintf(__('%s @ %s', 'polls'), get_option('date_format'), get_option('time_format')), gmdate('Y-m-d H:i:s', $poll_expiry));
	}
	...
	if($poll_question && $poll_answers) {
		...
		$temp_pollvote .= "<div id=\"polls-$poll_question_id\" class=\"polls\">\n";
		$temp_pollvote .= "\t<form id=\"polls_form_$poll_question_id\" class=\"polls-form\" action=\"".esc_attr($_SERVER['SCRIPT_NAME'])."\" method=\"post\">\n";
		$temp_pollvote .= "\t\t<p style=\"display: none;\"><input type=\"hidden\" id=\"poll_{$poll_question_id}_nonce\" name=\"polls-nonce\" value=\"".wp_create_nonce('poll_'.$poll_question_id.'-nonce')."\" /></p>\n";
		...
		if($display_loading) {
			...
			if(intval($poll_ajax_style['loading']) == 1) {
				$temp_pollvote .= "<div id=\"polls-$poll_question_id-loading\" class=\"polls-loading\"><img src=\"".plugins_url('polls/images/loading.gif')."\" width=\"16\" height=\"16\" alt=\"".__('Loading', 'polls')." ...\" title=\"".__('Loading', 'polls')." ...\" class=\"polls-image\" />&nbsp;".__('Loading', 'polls')." ...</div>\n";
			}
		}
	} else {
...
function display_pollresult($poll_id, $user_voted = '', $display_loading = true) {
	...
	if($poll_question && $poll_answers) {
		...
		$temp_pollresult .= "<div id=\"polls-$poll_question_id\" class=\"polls\">\n";
		...
		$temp_pollresult .= "\t\t<input type=\"hidden\" id=\"poll_{$poll_question_id}_nonce\" name=\"polls-nonce\" value=\"".wp_create_nonce('poll_'.$poll_question_id.'-nonce')."\" />\n";
		$temp_pollresult .= "</div>\n";
		if($display_loading) {
			$poll_ajax_style = get_option('poll_ajax_style');
			if(intval($poll_ajax_style['loading']) == 1) {
				$temp_pollresult .= "<div id=\"polls-$poll_question_id-loading\" class=\"polls-loading\"><img src=\"".plugins_url('polls/images/loading.gif')."\" width=\"16\" height=\"16\" alt=\"".__('Loading', 'polls')." ...\" title=\"".__('Loading', 'polls')." ...\" class=\"polls-image\" />&nbsp;".__('Loading', 'polls')." ...</div>\n";
			}
		}
	} else {
...
function poll_shortcode($atts) {
	...
	if( !is_feed() ) {
		...
		return __( 'Note: There is a poll embedded within this post, please visit the site to participate in this post\'s poll.', 'polls' );
	}
}
...
function polls_archive() {
	...
	// Poll Archives
	$pollsarchive_output_archive .= "<div class=\"polls polls-archive\">\n";
	foreach($polls_questions as $polls_question) {
		...
		if($polls_question['totalvotes'] > 0) {
			...
			$poll_start_date = mysql2date(sprintf(__('%s @ %s', 'polls'), get_option('date_format'), get_option('time_format')), gmdate('Y-m-d H:i:s', $polls_question['start']));
			if(empty($polls_question['end'])) {
				$poll_end_date  = __('No Expiry', 'polls');
			} else {
				$poll_end_date  = mysql2date(sprintf(__('%s @ %s', 'polls'), get_option('date_format'), get_option('time_format')), gmdate('Y-m-d H:i:s', $polls_question['end']));
			}
		// Archive Poll Header
	...
	if($max_page > 1) {
		...
		if(function_exists('wp_pagenavi')) {
			$pollsarchive_output_archive .= '<div class="pagenavi">'."\n";
		} else {
			$pollsarchive_output_archive .= '<div class="polls-paging">'."\n";
		}
		$pollsarchive_output_archive .= '<span class="pages">&#8201;'.sprintf(__('Page %s of %s', 'polls'), number_format_i18n($page), number_format_i18n($max_page)).'&#8201;</span>';
		if ($start_page >= 2 && $pages_to_show < $max_page) {
			$pollsarchive_output_archive .= '<a href="'.polls_archive_link(1).'" title="'.__('&laquo; First', 'polls').'">&#8201;'.__('&laquo; First', 'polls').'&#8201;</a>';
			$pollsarchive_output_archive .= '<span class="extend">...</span>';
		}
		if($page > 1) {
			$pollsarchive_output_archive .= '<a href="'.polls_archive_link(($page-1)).'" title="'.__('&laquo;', 'polls').'">&#8201;'.__('&laquo;', 'polls').'&#8201;</a>';
		}
		...
		if(empty($page) || ($page+1) <= $max_page) {
			$pollsarchive_output_archive .= '<a href="'.polls_archive_link(($page+1)).'" title="'.__('&raquo;', 'polls').'">&#8201;'.__('&raquo;', 'polls').'&#8201;</a>';
		}
		if ($end_page < $max_page) {
			$pollsarchive_output_archive .= '<span class="extend">...</span>';
			$pollsarchive_output_archive .= '<a href="'.polls_archive_link($max_page).'" title="'.__('Last &raquo;', 'polls').'">&#8201;'.__('Last &raquo;', 'polls').'&#8201;</a>';
		}
		$pollsarchive_output_archive .= '</div>';
...
function vote_poll() {
	...
	if(isset($_REQUEST['action']) && $_REQUEST['action'] == 'polls')
	{
		...
		{
			_e('Failed To Verify Referrer', 'polls');
			exit();
		}
					...
					if ( $is_poll_open > 0 ) {
						...
						if ($check_voted == 0) {
							...
							if ($vote_q) {
								...
								printf(__('Unable To Update Poll Total Votes And Poll Total Voters. Poll ID #%s', 'polls'), $poll_id);
							} // End if($vote_a)
						} else {
							printf(__('You Had Already Voted For This Poll. Poll ID #%s', 'polls'), $poll_id);
						} // End if($check_voted)
					} else {
						printf( __( 'Poll ID #%s is closed', 'polls' ), $poll_id );
					}  // End if($is_poll_open > 0)
				} else {
					printf(__('Invalid Poll ID. Poll ID #%s', 'polls'), $poll_id);
...
function manage_poll() {
	...
	if(isset($_POST['action']) && $_POST['action'] == 'polls-admin')
	{
		if(!empty($_POST['do'])) {
			...
			switch($_POST['do']) {
				// Delete Polls Logs
				case __('Delete All Logs', 'polls'):
					check_ajax_referer('polls_delete-polls-logs');
					if(trim($_POST['delete_logs_yes']) == 'yes') {
						$delete_logs = $wpdb->query("DELETE FROM $wpdb->pollsip");
						if($delete_logs) {
							echo '<p style="color: green;">'.__('All Polls Logs Have Been Deleted.', 'polls').'</p>';
						} else {
							echo '<p style="color: red;">'.__('An Error Has Occurred While Deleting All Polls Logs.', 'polls').'</p>';
						}
					}
					break;
				// Delete Poll Logs For Individual Poll
				case __('Delete Logs For This Poll Only', 'polls'):
					check_ajax_referer('polls_delete-poll-logs');
					...
					if(trim($_POST['delete_logs_yes']) == 'yes') {
						...
						if($delete_logs) {
							echo '<p style="color: green;">'.sprintf(__('All Logs For \'%s\' Has Been Deleted.', 'polls'), stripslashes($pollq_question)).'</p>';
						} else {
							echo '<p style="color: red;">'.sprintf(__('An Error Has Occurred While Deleting All Logs For \'%s\'', 'polls'), stripslashes($pollq_question)).'</p>';
						}
					}
					break;
				// Delete Poll's Answer
				case __('Delete Poll Answer', 'polls'):
					check_ajax_referer('polls_delete-poll-answer');
					$pollq_id  = intval($_POST['pollq_id']);
					...
					if($delete_polla_answers) {
						echo '<p style="color: green;">'.sprintf(__('Poll Answer \'%s\' Deleted Successfully.', 'polls'), $polla_answers).'</p>';
					} else {
						echo '<p style="color: red;">'.sprintf(__('Error In Deleting Poll Answer \'%s\'.', 'polls'), $polla_answers).'</p>';
					}
					break;
				// Open Poll
				case __('Open Poll', 'polls'):
					check_ajax_referer('polls_open-poll');
					$pollq_id  = intval($_POST['pollq_id']);
					$pollq_question = $wpdb->get_var("SELECT pollq_question FROM $wpdb->pollsq WHERE pollq_id = $pollq_id");
					$open_poll = $wpdb->query("UPDATE $wpdb->pollsq SET pollq_active = 1 WHERE pollq_id = $pollq_id;");
					if($open_poll) {
						echo '<p style="color: green;">'.sprintf(__('Poll \'%s\' Is Now Opened', 'polls'), stripslashes($pollq_question)).'</p>';
					} else {
						echo '<p style="color: red;">'.sprintf(__('Error Opening Poll \'%s\'', 'polls'), stripslashes($pollq_question)).'</p>';
					}
					break;
				// Close Poll
				case __('Close Poll', 'polls'):
					check_ajax_referer('polls_close-poll');
					$pollq_id  = intval($_POST['pollq_id']);
					$pollq_question = $wpdb->get_var("SELECT pollq_question FROM $wpdb->pollsq WHERE pollq_id = $pollq_id");
					$close_poll = $wpdb->query("UPDATE $wpdb->pollsq SET pollq_active = 0 WHERE pollq_id = $pollq_id;");
					if($close_poll) {
						echo '<p style="color: green;">'.sprintf(__('Poll \'%s\' Is Now Closed', 'polls'), stripslashes($pollq_question)).'</p>';
					} else {
						echo '<p style="color: red;">'.sprintf(__('Error Closing Poll \'%s\'', 'polls'), stripslashes($pollq_question)).'</p>';
					}
					break;
				// Delete Poll
				case __('Delete Poll', 'polls'):
					check_ajax_referer('polls_delete-poll');
					$pollq_id  = intval($_POST['pollq_id']);
					$pollq_question = $wpdb->get_var("SELECT pollq_question FROM $wpdb->pollsq WHERE pollq_id = $pollq_id");
					...
					if(!$delete_poll_question) {
						echo '<p style="color: red;">'.sprintf(__('Error In Deleting Poll \'%s\' Question', 'polls'), stripslashes($pollq_question)).'</p>';
					}
					if(empty($text)) {
						echo '<p style="color: green;">'.sprintf(__('Poll \'%s\' Deleted Successfully', 'polls'), stripslashes($pollq_question)).'</p>';
					}
...
### Function: Add Polls General Stats To Stats Page Options
function polls_page_admin_general_stats($content) {
	$stats_display = get_option('stats_display');
	if($stats_display['polls'] == 1) {
		$content .= '<input type="checkbox" name="stats_display[]" id="wpstats_polls" value="polls" checked="checked" />&nbsp;&nbsp;<label for="wpstats_polls">'.__('Polls', 'polls').'</label><br />'."\n";
	} else {
		$content .= '<input type="checkbox" name="stats_display[]" id="wpstats_polls" value="polls" />&nbsp;&nbsp;<label for="wpstats_polls">'.__('Polls', 'polls').'</label><br />'."\n";
	}
	return $content;
}


### Function: Add Polls General Stats To Stats Page
function polls_page_general_stats($content) {
	$stats_display = get_option('stats_display');
	if($stats_display['polls'] == 1) {
		$content .= '<p><strong>'.__('Polls', 'polls').'</strong></p>'."\n";
		$content .= '<ul>'."\n";
		$content .= '<li>'.sprintf(_n('<strong>%s</strong> poll was created.', '<strong>%s</strong> polls were created.', get_pollquestions(false), 'polls'), number_format_i18n(get_pollquestions(false))).'</li>'."\n";
		$content .= '<li>'.sprintf(_n('<strong>%s</strong> polls\' answer was given.', '<strong>%s</strong> polls\' answers were given.', get_pollanswers(false), 'polls'), number_format_i18n(get_pollanswers(false))).'</li>'."\n";
		$content .= '<li>'.sprintf(_n('<strong>%s</strong> vote was cast.', '<strong>%s</strong> votes were cast.', get_pollvotes(false), 'polls'), number_format_i18n(get_pollvotes(false))).'</li>'."\n";
		$content .= '</ul>'."\n";
	}
	return $content;
}
...
### Class: Polls Widget
 class WP_Widget_Polls extends WP_Widget {
	...
	function WP_Widget_Polls() {
		$widget_ops = array('description' => __('Polls polls', 'polls'));
		$this->WP_Widget('polls-widget', __('Polls', 'polls'), $widget_ops);
	}
	...
	function form($instance) {
		global $wpdb;
		$instance = wp_parse_args((array) $instance, array('title' => __('Polls', 'polls'), 'poll_id' => 0, 'display_pollarchive' => 1));
		$title = esc_attr($instance['title']);
		$poll_id = intval($instance['poll_id']);
		$display_pollarchive = intval($instance['display_pollarchive']);
?>
		<p>
			<label for="<?php echo $this->get_field_id('title'); ?>"><?php _e('Title:', 'polls'); ?> <input class="widefat" id="<?php echo $this->get_field_id('title'); ?>" name="<?php echo $this->get_field_name('title'); ?>" type="text" value="<?php echo $title; ?>" /></label>
		</p>
		<p>
			<label for="<?php echo $this->get_field_id('display_pollarchive'); ?>"><?php _e('Display Polls Archive Link Below Poll?', 'polls'); ?>
				<select name="<?php echo $this->get_field_name('display_pollarchive'); ?>" id="<?php echo $this->get_field_id('display_pollarchive'); ?>" class="widefat">
					<option value="0"<?php selected(0, $display_pollarchive); ?>><?php _e('No', 'polls'); ?></option>
					<option value="1"<?php selected(1, $display_pollarchive); ?>><?php _e('Yes', 'polls'); ?></option>
				</select>
			</label>
		</p>
		<p>
			<label for="<?php echo $this->get_field_id('poll_id'); ?>"><?php _e('Poll To Display:', 'polls'); ?>
				<select name="<?php echo $this->get_field_name('poll_id'); ?>" id="<?php echo $this->get_field_id('poll_id'); ?>" class="widefat">
					<option value="-1"<?php selected(-1, $poll_id); ?>><?php _e('Do NOT Display Poll (Disable)', 'polls'); ?></option>
					<option value="-2"<?php selected(-2, $poll_id); ?>><?php _e('Display Random Poll', 'polls'); ?></option>
					<option value="0"<?php selected(0, $poll_id); ?>><?php _e('Display Latest Poll', 'polls'); ?></option>
					<optgroup>&nbsp;</optgroup>
					<?php
...
function polls_activate() {
	...
	if(@is_file(ABSPATH.'/admin/upgrade-functions.php')) {
		include_once(ABSPATH.'/admin/upgrade-functions.php');
	} elseif(@is_file(ABSPATH.'/admin/includes/upgrade.php')) {
		include_once(ABSPATH.'/admin/includes/upgrade.php');
	} else {
		die('We have problem finding your \'/admin/upgrade-functions.php\' and \'/admin/includes/upgrade.php\'');
	}
	...
	if(empty($first_poll)) {
		...
		$insert_pollq = $wpdb->query("INSERT INTO $wpdb->pollsq VALUES (1, '".__('How Is My Site?', 'polls')."', '".current_time('timestamp')."', 0, 1, '', 0, 0);");
		if($insert_pollq) {
			// Insert Poll Answers  (5 Records)
			$wpdb->query("INSERT INTO $wpdb->pollsa VALUES (1, 1, '".__('Good', 'polls')."', 0);");
			$wpdb->query("INSERT INTO $wpdb->pollsa VALUES (2, 1, '".__('Excellent', 'polls')."', 0);");
			$wpdb->query("INSERT INTO $wpdb->pollsa VALUES (3, 1, '".__('Bad', 'polls')."', 0);");
			$wpdb->query("INSERT INTO $wpdb->pollsa VALUES (4, 1, '".__('Can Be Improved', 'polls')."', 0);");
			$wpdb->query("INSERT INTO $wpdb->pollsa VALUES (5, 1, '".__('No Comments', 'polls')."', 0);");
		}
	}
	...
	'<p style="text-align: center;"><input type="button" name="vote" value="   '.__('Vote', 'polls').'   " class="Buttons" onclick="poll_vote(%POLL_ID%);" /></p>'.
	'<p style="text-align: center;"><a href="#ViewPollResults" onclick="poll_result(%POLL_ID%); return false;" title="'.__('View Results Of This Poll', 'polls').'">'.__('View Results', 'polls').'</a></p>'.
	'</div>');
	add_option('poll_template_resultheader', '<p style="text-align: center;"><strong>%POLL_QUESTION%</strong></p>'.
	'<div id="polls-%POLL_ID%-ans" class="polls-ans">'.
	'<ul class="polls-ul">');
	add_option('poll_template_resultbody', '<li>%POLL_ANSWER% <small>(%POLL_ANSWER_PERCENTAGE%%'.__(',', 'polls').' %POLL_ANSWER_VOTES% '.__('Votes', 'polls').')</small><div class="pollbar" style="width: %POLL_ANSWER_IMAGEWIDTH%%;" title="%POLL_ANSWER_TEXT% (%POLL_ANSWER_PERCENTAGE%% | %POLL_ANSWER_VOTES% '.__('Votes', 'polls').')"></div></li>');
	add_option('poll_template_resultbody2', '<li><strong><i>%POLL_ANSWER% <small>(%POLL_ANSWER_PERCENTAGE%%'.__(',', 'polls').' %POLL_ANSWER_VOTES% '.__('Votes', 'polls').')</small></i></strong><div class="pollbar" style="width: %POLL_ANSWER_IMAGEWIDTH%%;" title="'.__('You Have Voted For This Choice', 'polls').' - %POLL_ANSWER_TEXT% (%POLL_ANSWER_PERCENTAGE%% | %POLL_ANSWER_VOTES% '.__('Votes', 'polls').')"></div></li>');
	add_option('poll_template_resultfooter', '</ul>'.
	'<p style="text-align: center;">'.__('Total Voters', 'polls').': <strong>%POLL_TOTALVOTERS%</strong></p>'.
	'</div>');
	add_option('poll_template_resultfooter2', '</ul>'.
	'<p style="text-align: center;">'.__('Total Voters', 'polls').': <strong>%POLL_TOTALVOTERS%</strong></p>'.
	'<p style="text-align: center;"><a href="#VotePoll" onclick="poll_booth(%POLL_ID%); return false;" title="'.__('Vote For This Poll', 'polls').'">'.__('Vote', 'polls').'</a></p>'.
	'</div>');
	add_option('poll_template_disable', __('Sorry, there are no polls available at the moment.', 'polls'));
	add_option('poll_template_error', __('An error has occurred when processing your poll.', 'polls'));
	...
	// Database Upgrade For Polls 2.1
	...
	// Database Upgrade For Polls 2.12
	...
	// Database Upgrade For Polls 2.20
	add_option('poll_ajax_style', array('loading' => 1, 'fading' => 1));
	add_option('poll_template_pollarchivelink', '<ul>'.
	'<li><a href="%POLL_ARCHIVE_URL%">'.__('Polls Archive', 'polls').'</a></li>'.
	'</ul>');
	add_option('poll_archive_displaypoll', 2);
	add_option('poll_template_pollarchiveheader', '');
	add_option('poll_template_pollarchivefooter', '<p>'.__('Start Date:', 'polls').' %POLL_START_DATE%<br />'.__('End Date:', 'polls').' %POLL_END_DATE%</p>');
	...
	// Database Upgrade For Polls 2.30
	...
	// Database Upgrade For Polls 2.50
