...
= Build Status =
[![Build Status](https://travis-ci.org/lesterchan/polls.svg?branch=master)](https://travis-ci.org/lesterchan/polls)

= Development =
[https://github.com/lesterchan/polls](https://github.com/lesterchan/polls "https://github.com/lesterchan/polls")

= Translations =
[http://dev.plugins.org/browser/polls/i18n/](http://dev.plugins.org/browser/polls/i18n/ "http://dev.plugins.org/browser/polls/i18n/")
...
* Move AJAX Request to admin/admin-ajax.php
...
* FIXED: More Efficient Polls Archive
...
* NEW: Updated Polls TinyMCE Plugin To Work With TinyMCE 3.0
* NEW: Add Time Expiry For Cookie/Log
* NEW: Removed polls-usage.php
* NEW: Removed "Fade Anything Technique" In Polls Admin
* NEW: Uses /polls/ Folder Instead Of /polls/
* NEW: Uses polls.php Instead Of polls.php
* NEW: Uses polls-widget.php Instead Of polls-widget.php
...
* NEW: Poll Templates Has Now Its Own Page 'Admin -> Polls -> Poll Templates'
...
* NEW: Major Changes To The Administration Panel For Polls
* NEW: AJAX Added To The Administration Panel For Polls
...
* FIXED: Added Some Default Styles To polls-css.css To Ensure That Polls Does Not Break
...
* NEW: Polls Is Now Using DIV To Display The Poll's Results Instead Of The Image Bar
* NEW: Added Widget Title Option To Polls Widget
* NEW: Ability To Logged By UserName
* NEW: Added CSS Class 'polls-image' To All IMG Tags
* FIXED: If Site URL Doesn't Match WP Option's Site URL, Polls Will Not Work
...
* NEW: Poll Administration Panel And The Code That Polls Generated Is XHTML 1.0 Transitional
...
* FIXED: Totally Removed Tables In polls.php
...
* NEW: Page Title Added To polls.php
...
2. Put: `Folder: polls`
3. Activate `Polls` Plugin
4. Go to `Admin -> Polls`

= General Usage (Without Widget) =
1. Open `content/themes/<YOUR THEME NAME>/sidebar.php`
2. Add:
<code>
...
1. Deactivate `Polls` Plugin
2. Open `content/plugins` Folder
3. Put/Overwrite: `Folder: polls`
4. Activate `Polls` Plugin
5. Go to `Admin -> Polls -> Polls Templates` and restore all the template variables to `Default`
6. Go to `Admin -> Appearance -> Widgets` and re-add the Poll Widget
...
* If you ARE NOT using nice permalinks, you need to go to `Admin -> Polls -> Poll Options` and under `Poll Archive -> Polls Archive URL`, you need to fill in the URL to the Polls Archive Page you created above.
...
= How Does Polls Load CSS? =
* Polls will load `polls-css.css` from your theme's directory if it exists.
* If it doesn't exists, it will just load the default `polls-css.css` that comes with Polls.
* This will allow you to upgrade Polls without worrying about overwriting your polls styles that you have created.
...
<code>
.polls-ul li:nth-child(01) .pollbar{ background:#8FA0C5}
.polls-ul li:nth-child(02) .pollbar{ background:#FF8}
.polls-ul li:nth-child(03) .pollbar{ background:#ff8a3b}
.polls-ul li:nth-child(04) .pollbar{ background:#a61e2a}
.polls-ul li:nth-child(05) .pollbar{ background:#4ebbff}
.polls-ul li:nth-child(06) .pollbar{ background:#fbca54}
.polls-ul li:nth-child(07) .pollbar{ background:#aad34f}
.polls-ul li:nth-child(08) .pollbar{ background:#66cc9a}
.polls-ul li:nth-child(09) .pollbar{ background:#98CBCB}
.polls-ul li:nth-child(10) .pollbar{ background:#a67c52}
.polls-ul li .pollbar{ transition: background 0.7s ease-in-out }
.polls-ul li .pollbar:hover{ background:#F00 }
</code>
