...
if(!empty($_POST['do'])) {
	...
	switch($_POST['do']) {
		// Add Poll
		case __('Add Poll', 'polls'):
			check_admin_referer('polls_add-poll');
			// Poll Question
			...
			if( ! empty( $pollq_question ) ) {
				...
				if (!$add_poll_question) {
					$text .= '<p style="color: red;">' . sprintf(__('Error In Adding Poll \'%s\'.', 'polls'), stripslashes($pollq_question)) . '</p>';
				}
				// Add Poll Answers
				...
				foreach ($polla_answers as $polla_answer) {
					...
					if( ! empty( $polla_answer ) ) {
						...
						if (!$add_poll_answers) {
							$text .= '<p style="color: red;">' . sprintf(__('Error In Adding Poll\'s Answer \'%s\'.', 'polls'), stripslashes($polla_answer)) . '</p>';
						}
					} else {
						$text .= '<p style="color: red;">' . __( 'Poll\'s Answer is empty.', 'polls' ) . '</p>';
					}
				}
				...
				if ( empty( $text ) ) {
					$text = '<p style="color: green;">' . sprintf( __( 'Poll \'%s\' (ID: %s) added successfully. Embed this poll with the shortcode: %s or go back to <a href="%s">Manage Polls</a>', 'polls' ), stripslashes( $pollq_question ), $latest_pollid, '<input type="text" value=\'[poll id="' . $latest_pollid . '"]\' readonly="readonly" size="10" />', $base_page ) . '</p>';
				} else {
					if( $add_poll_question ) {
						$text .= '<p style="color: green;">' . sprintf( __( 'Poll \'%s\' (ID: %s) (Shortcode: %s) added successfully, but there are some errors with the Poll\'s Answers. Embed this poll with the shortcode: %s or go back to <a href="%s">Manage Polls</a>', 'polls' ), stripslashes( $pollq_question ), $latest_pollid, '<input type="text" value=\'[poll id="' . $latest_pollid . '"]\' readonly="readonly" size="10" />' ) .'</p>';
					}
				}
				cron_polls_place();
			} else {
				$text .= '<p style="color: red;">' . __( 'Poll Question is empty.', 'polls' ) . '</p>';
			}
			break;
	}
}
...
<?php wp_nonce_field('polls_add-poll'); ?>
<div class="wrap">
	<h2><?php _e('Add Poll', 'polls'); ?></h2>
	<!-- Poll Question -->
	...
	<table class="form-table">
		<tfoot>
			<tr>
				<td width="20%">&nbsp;</td>
				<td width="80%"><input type="button" value="<?php _e('Add Answer', 'polls') ?>" onclick="add_poll_answer_add();" class="button" /></td>
			</tr>
		</tfoot>
		...
		<?php
			for($i = 1; $i <= $poll_noquestion; $i++) {
				...
				echo "<td width=\"80%\"><input type=\"text\" size=\"50\" maxlength=\"200\" name=\"polla_answers[]\" />&nbsp;&nbsp;&nbsp;<input type=\"button\" value=\"".__('Remove', 'polls')."\" onclick=\"remove_poll_answer_add(".$i.");\" class=\"button\" /></td>\n";
				echo "</tr>\n";
				$count++;
			}
		?>
		...
		<tr>
			...
			<td width="60%">
				<select name="pollq_multiple_yes" id="pollq_multiple_yes" size="1" onchange="check_pollq_multiple();">
					<option value="0"><?php _e('No', 'polls'); ?></option>
					<option value="1"><?php _e('Yes', 'polls'); ?></option>
				</select>
			</td>
		</tr>
		<tr>
	...
	<table class="form-table">
		...
		<tr>
			...
			<td width="80%"><input type="checkbox" name="pollq_expiry_no" id="pollq_expiry_no" value="1" checked="checked" onclick="check_pollexpiry();" />&nbsp;&nbsp;<label for="pollq_expiry_no"><?php _e('Do NOT Expire This Poll', 'polls'); ?></label><?php poll_timestamp(current_time('timestamp'), 'pollq_expiry', 'none'); ?></td>
		</tr>
	</table>
	<p style="text-align: center;"><input type="submit" name="do" value="<?php _e('Add Poll', 'polls'); ?>"  class="button-primary" />&nbsp;&nbsp;<input type="button" name="cancel" value="<?php _e('Cancel', 'polls'); ?>" class="button" onclick="javascript:history.go(-1)" /></p>
</div>
</form>