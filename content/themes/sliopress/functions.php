...
if ( ! function_exists( 'T20_load' ) ) {
	...
	function T20_load() {
		...
		if ( ot_get_option('wp_pagenavi') == 'on' ) {
			load_template( get_template_directory() . '/admin/pagenavi.php' );
		}
...
function time_ago( $type = 'post' ) {
	...
	return human_time_diff($d('U'), current_time('timestamp')) . " " . ot_get_option('news_ago');
}
...
if ( ! function_exists( 'T20_site_title' ) ) {
	...
	function T20_site_title() {
		...
		if ( ot_get_option('custom-logo') ) {
			$logo = '<a href="'.home_url('/').'" rel="home" title="'.get_bloginfo('description').'"><img src="'.ot_get_option('custom-logo').'" alt="'.get_bloginfo('name').'"></a>';
		} else {
...
/* ------------------------------------ */
if ( ! function_exists( 'T20_meta_seo' ) ) {

	function T20_meta_seo() {
		if ( is_single() || is_page() ) { 
			...
			?><meta name="keywords" content="<?php 
				...
				} elseif ( ot_get_option('meta-keywords-default') ) {
					echo ot_get_option('meta-keywords-default');
				}
			echo '" />'."\n";
		} else {
			if ( ot_get_option('meta-description-default') ) {
				echo '<meta name="description" content="'.ot_get_option('meta-description-default').'">'."\n";
			}
			if ( ot_get_option('meta-keywords-default') ) {
				echo '<meta name="keywords" content="'.ot_get_option('meta-keywords-default').'">'."\n";
			}
		}
	}
	
}
add_filter( 'wp_head', 'T20_meta_seo' );
	...
	function T20_plugins() {
		$plugins = array(
			...
			array(
				...
				'slug' 			=> 'ajax-login',
			            'source'   		=> get_stylesheet_directory() . '/plugins/ajax-login.zip', 
				'required' 		=> false, 
				'force_activation' 	=> false, 
				'force_deactivation' 	=> false, 
			),
...
if (class_exists('WPBakeryVisualComposerAbstract')) {
	...
	function my_custom_vc() {
		echo '<style>.wpb_switch-to-front-composer, #vc_teaser {display: none}</style>';
	}
