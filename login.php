...
/** Make sure that the SlioPress bootstrap has run before continuing. */
 ...
 *
 * @param string   $title    Optional. SlioPress login Page title to display in the `<title>` element.
 *                           Default 'Log In'.
...
function login_header( $title = 'Log In', $message = '', $wp_error = '' ) {
	...
	if ( is_multisite() ) {
		...
		$login_header_title = __( 'Powered by SlioPress' );
	}
...
case 'postpass' :
	...
	setcookie( 'postpass_' . COOKIEHASH, $hasher->HashPassword( wp_unslash( $_POST['post_password'] ) ), $expire, COOKIEPATH, COOKIE_DOMAIN, $secure );
...
<form name="lostpasswordform" id="lostpasswordform" action="<?php echo esc_url( network_site_url( 'login.php?action=lostpassword', 'login_post' ) ); ?>" method="post">
	...
	<p class="submit"><input type="submit" name="submit" id="submit" class="button button-primary button-large" value="<?php esc_attr_e('Get New Password'); ?>" /></p>
</form>
...
<form name="resetpassform" id="resetpassform" action="<?php echo esc_url( network_site_url( 'login.php?action=resetpass', 'login_post' ) ); ?>" method="post" autocomplete="off">
	...
	<p class="submit"><input type="submit" name="submit" id="submit" class="button button-primary button-large" value="<?php esc_attr_e('Reset Password'); ?>" /></p>
</form>
...
<form name="registerform" id="registerform" action="<?php echo esc_url( site_url('login.php?action=register', 'login_post') ); ?>" method="post" novalidate="novalidate">
	...
	<br class="clear" />
	<input type="hidden" name="redirect_to" value="<?php echo esc_attr( $redirect_to ); ?>" />
	<p class="submit"><input type="submit" name="submit" id="submit" class="button button-primary button-large" value="<?php esc_attr_e('Register'); ?>" /></p>
</form>
...
default:
	...
	if ( empty( $_COOKIE[ LOGGED_IN_COOKIE ] ) ) {
		if ( headers_sent() ) {
			...
			$user = new WP_Error( 'test_cookie', sprintf( __( '<strong>ERROR</strong>: Cookies are blocked or not supported by your browser. You must <a href="%s">enable cookies</a> to use SlioPress.' ),
...
<?php		exit;
	...
	if ( $interim_login ) {
		...
		elseif ( strpos( $redirect_to, 'about.php?updated' ) )
			$errors->add('updated', __( '<strong>You have successfully updated SlioPress!</strong> Please log back in to see what&#8217;s new.' ), 'message' );
	}
