...
tinymce.PluginManager.add( 'wpeditimage', function( editor ) {
	...
	function toolbarConfig() {
		...
		return {
			type: 'panel',
			layout: 'stack',
			classes: 'toolbar-grp inline-toolbar-grp image-toolbar',
			ariaRoot: true,
			ariaRemember: true,
			items: [
				{
	...
	DOM.bind( window, 'resize scroll', function() {
		if ( ! toolbarIsHidden && editorWrapParent.hasClass( 'editor-expand' ) ) {
			hide();
		}
	});
	...
	editor.on( 'init', function() {
		...
		editor.on( 'wpNewImageRefresh', function( event ) {
			...
			if ( parent = dom.getParent( event.node, 'dl.caption' ) ) {
		...
		editor.on( 'wpImageFormSubmit', function( event ) {
			...
			if ( ! imgNode ) {
				...
				if ( caption ) {
					...
					html = '<dl class="caption alignnone"' + captionWidth + '>' +
						'<dt class="caption-dt">'+ html +'</dt><dd class="caption-dd">'+ caption +'</dd></dl>';
				...
				wrap = dom.getParent( imgNode, 'dl.caption' );

				if ( caption ) {
					if ( wrap ) {
						if ( parent = dom.select( 'dd.caption-dd', wrap )[0] ) {
							parent.innerHTML = caption;
						}
					} else {
						if ( imgNode.className ) {
							captionId = imgNode.className.match( /image-([0-9]+)/ );
						...
						captionAlign = ' class="caption ' + captionAlign + '"';
						...
						html = '<dl ' + captionId + captionAlign + captionWidth + '>' +
							'<dt class="caption-dt"></dt><dd class="caption-dd">'+ caption +'</dd></dl>';
		...
		editor.on( 'wpLoadImageData', function( event ) {
			...
			if ( parent = dom.getParent( imgNode, 'dl.caption' ) ) {
				parent = dom.select( 'dd.caption-dd', parent )[0];
		...
		dom.bind( editor.getDoc(), 'dragstart', function( event ) {
			...
			if ( node.nodeName === 'IMG' && dom.getParent( node, '.caption' ) ) {
