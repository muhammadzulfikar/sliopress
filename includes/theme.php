...
function _custom_header_background_just_in_time() {
	...
	if ( current_theme_supports( 'custom-header' ) ) {
		...
		if ( $args[0]['head-callback'] )
			add_action( 'wp_head', $args[0]['head-callback'] );
...
function _remove_theme_support( $feature ) {
	...
	switch ( $feature ) {
		case 'custom-header' :
			...
			if ( $support[0]['head-callback'] )
				remove_action( 'wp_head', $support[0]['head-callback'] );
 ...
 *
 * Fires when ?wp_customize=on or on admin/customize.php.
 *
 * @since 3.4.0
 */
function _wp_customize_include() {
	...
	require( ABSPATH . WPINC . '/class-customize-manager.php' );
