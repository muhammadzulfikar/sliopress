...
final class _WP_Editors {
	...
	public static function editor( $content, $editor_id, $settings = array() ) {
		...
		$editor_class = ' class="' . trim( $set['editor_class'] . ' editor-area' ) . '"';
		...
		if ( ! self::$this_quicktags && self::$this_tinymce ) {
			...
			$buttons .= '<button type="button" id="' . $editor_id . '-tmce" class="switch-editor switch-tmce" onclick="switchEditors.switchto(this);">' . __('Visual') . "</button>\n";
			$buttons .= '<button type="button" id="' . $editor_id . '-html" class="switch-editor switch-html" onclick="switchEditors.switchto(this);">' . _x( 'Text', 'Name for the Text editor tab (formerly HTML)' ) . "</button>\n";
		}
		...
		echo '<div id="' . $editor_id . '-wrap" class="' . $wrap_class . '">';
		...
		if ( !empty($buttons) || $set['media_buttons'] ) {
			echo '<div id="' . $editor_id . '-editor-tools" class="editor-tools hide-if-no-js">';
			...
			if ( $set['media_buttons'] ) {
				...
				echo '<div id="' . $editor_id . '-media-buttons" class="media-buttons">';
		 ...
		 */
		$the_editor = apply_filters( 'the_editor', '<div id="' . $editor_id . '-editor-container" class="editor-container">' .
		...
		if ( self::$this_tinymce ) {
			...
			if ( empty( self::$first_init ) ) {
				...
				self::$first_init = array(
					...
					'entity_encoding' => 'raw',
					'keep_styles' => false,
					'cache_suffix' => 'mce-' . $GLOBALS['tinymce_version'],
	...
	public static function editor_js() {
		...
		// Load tinymce.js when running from /src, else load tinymce.js.gz (production) or tinymce.min.js (SCRIPT_DEBUG)
		$mce_suffix = false !== strpos( $GLOBALS['wp_version'], '-src' ) ? '' : '.min';

		if ( $tmce_on ) {
			if ( $compressed ) {
				echo "<script type='text/javascript' src='{$baseurl}/tinymce.php?c=1&amp;$version'></script>\n";
			} else {
		...
		( function() {
			...
			if ( typeof tinymce !== 'undefined' ) {
				for ( edId in tinyMCEPreInit.mceInit ) {
					...
					wrapper = tinymce.DOM.select( '#' + edId + '-wrap' )[0];
			...
			if ( typeof jQuery !== 'undefined' ) {
				jQuery('.editor-wrap').on( 'click.editor', function() {
					if ( this.id ) {
				...
				for ( qtId in tinyMCEPreInit.qtInit ) {
					document.getElementById( '' + qtId + '-wrap' ).onclick = function() {
						window.wpActiveEditor = this.id.slice( 3, -5 );
					}
				}
			}
	...
	public static function wp_fullscreen_html() {
		...
		?>
		<div id="fullscreen-body" class="core-ui<?php if ( is_rtl() ) echo ' rtl'; ?>" data-theme-width="<?php echo (int) $width; ?>" data-dfw-width="<?php echo (int) $dfw_width; ?>">
		<div id="fullscreen-topbar">
			<div id="fullscreen-toolbar">
			<div id="fullscreen-close"><a href="#" onclick="wp.editor.fullscreen.off();return false;"><?php _e('Exit fullscreen'); ?></a></div>
			<div id="fullscreen-central-toolbar" style="width:<?php echo $width; ?>px;">

			<div id="fullscreen-mode-bar">
				<div id="fullscreen-modes" class="button-group">
					<a class="button fullscreen-mode-tinymce" href="#" onclick="wp.editor.fullscreen.switchmode( 'tinymce' ); return false;"><?php _e( 'Visual' ); ?></a>
					<a class="button fullscreen-mode-html" href="#" onclick="wp.editor.fullscreen.switchmode( 'html' ); return false;"><?php _ex( 'Text', 'Name for the Text editor tab (formerly HTML)' ); ?></a>
				</div>
			</div>

			<div id="fullscreen-button-bar"><div id="fullscreen-buttons" class="mce-toolbar">
		<?php
		...
		$buttons = array(
			...
			'media-library' => array( 'title' => __('Media library (Alt + Shift + M)'), 'both' => true ),
		...
		foreach ( $buttons as $button => $args ) {
			...
			<div class="mce-widget mce-btn<?php if ( $args['both'] ) { ?> fullscreen-both<?php } ?>">
		...
		<div id="fullscreen-save">
			<input type="button" class="button button-primary right" value="<?php echo $save; ?>" onclick="wp.editor.fullscreen.save();" />
			<span class="fullscreen-saved-message"><?php if ( $post->post_status == 'publish' ) _e('Updated.'); else _e('Saved.'); ?></span>
			<span class="fullscreen-error-message"><?php _e('Save failed.'); ?></span>
	...
	<div id="fullscreen-statusbar">
		<div id="fullscreen-status">
			<div id="fullscreen-count"><?php printf( __( 'Word count: %s' ), '<span class="word-count">0</span>' ); ?></div>
			<div id="fullscreen-tagline"><?php _e('Just write.'); ?></div>
		</div>
	</div>
	</div>
	...
	public static function wp_link_dialog() {
		...
		<div id="link-wrap" class="core-ui<?php echo $search_panel_visible; ?>" style="display: none">
		...
		<div id="link-selector">
			...
			<p class="howto"><a href="#" id="link-search-toggle"><?php _e( 'Or link to existing content' ); ?></a></p>
			<div id="search-panel">
