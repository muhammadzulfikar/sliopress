...
$( document ).on( 'heartbeat-tick.check-locked-posts', function( e, data ) {
	var locked = data['check-locked-posts'] || {};
	...
	$('#the-list tr').each( function(i, el) {
		...
		if ( locked.hasOwnProperty( key ) ) {
			...
			row.removeClass('locked').delay(1000).find('.locked-info span').empty();
		}
	});
}).on( 'heartbeat-send.check-locked-posts', function( e, data ) {
	var check = [];
