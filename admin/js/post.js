...
$(document).on( 'heartbeat-send.refresh-lock', function( e, data ) {
	...
	if ( data['refresh-post-lock'] ) {
		received = data['refresh-post-lock'];
...
(function($) {
	...
	$(document).on( 'heartbeat-send.refresh-nonces', function( e, data ) {
		var nonce, post_id;

		if ( check ) {
			if ( ( post_id = $('#post_ID').val() ) && ( nonce = $('#_wpnonce').val() ) ) {
				data['refresh-post-nonces'] = {
					post_id: post_id,
					post_nonce: nonce
				};
			}
		}
	}).on( 'heartbeat-tick.refresh-nonces', function( e, data ) {
		var nonces = data['refresh-post-nonces'];
...
jQuery(document).ready( function($) {
	...
	$('#post-lock-dialog .notification-dialog').on( 'keydown', function(e) {
		...
		if ( target.hasClass('tab-first') && e.shiftKey ) {
			$(this).find('.tab-last').focus();
			e.preventDefault();
		} else if ( target.hasClass('tab-last') && ! e.shiftKey ) {
			$(this).find('.tab-first').focus();
			e.preventDefault();
		}
	}).filter(':visible').find('.tab-first').focus();
	...
	$('#post-preview').on( 'click.post-preview', function( event ) {
		var $this = $(this),
			$form = $('form#post'),
			$previewField = $('input#preview'),
			target = $this.attr('target') || 'preview',
	...
	$('.categorydiv').each( function(){
		...
		// TODO: move to jQuery 1.3+, support for multiple hierarchical taxonomies, see lists.js
		...
		$('#' + taxonomy + '-add-toggle').click( function() {
			$('#' + taxonomy + '-adder').toggleClass( 'hidden-children' );
	...
	( function() {
		...
		function dragging( event ) {
			if ( $postdivrich.hasClass( 'editor-expand' ) ) {
				return;
			}
		...
		$handle.on( 'mousedown.editor-resize', function( event ) {
			...
			$document.on( 'mousemove.editor-resize', dragging )
				.on( 'mouseup.editor-resize mouseleave.editor-resize', endDrag );

			event.preventDefault();
		}).on( 'mouseup.editor-resize', endDrag );
	})();
