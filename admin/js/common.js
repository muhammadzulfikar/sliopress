...
$(document).ready( function() {
	...
	$adminmenu.on('click.submenu-head', '.submenu-head', function(e){
	...
	$('#collapse-menu').on('click.collapse-menu', function() {
		...
		$('#adminmenu div.submenu').css('margin-top', '');
		...
		$( document ).trigger( 'collapse-menu', { state: state } );
	});
	...
	if ( 'ontouchstart' in window || /IEMobile\/[1-9]/.test(navigator.userAgent) ) { // touch screen device
		...
		$adminmenu.find( 'a.has-submenu' ).on( mobileEvent + '.mobile-hover', function( event ) {
			...
			if ( ! $menuItem.hasClass( 'opensub' ) && ( ! $menuItem.hasClass( 'menu-open' ) || $menuItem.width() < 40 ) ) {
	...
	if ( ! isIOS && ! isAndroid ) {
		$adminmenu.find( 'li.has-submenu' ).hoverIntent({
			over: function() {
				var $menuItem = $( this ),
					$submenu = $menuItem.find( '.submenu' ),
			...
			out: function(){
				...
				$( this ).removeClass( 'opensub' ).find( '.submenu' ).css( 'margin-top', '' );
			},
			timeout: 200,
			sensitivity: 7,
			interval: 90
		...
		$adminmenu.on( 'focus.adminmenu', '.submenu a', function( event ) {
			if ( $adminmenu.data( 'responsive' ) ) {
				// The menu is in responsive mode, bail
				return;
			}
		...
		}).on( 'blur.adminmenu', '.submenu a', function( event ) {
			if ( $adminmenu.data( 'responsive' ) ) {
				return;
			}

			$( event.target ).closest( 'li.menu-top' ).removeClass( 'opensub' );
		}).find( 'li.has-submenu.not-current-submenu' ).on( 'focusin.adminmenu', function() {
			adjustSubmenu( $( this ) );
		});
	}
	...
	$('thead, tfoot').find('.check-column :checkbox').on( 'click.toggle-checkboxes', function( event ) {
		var $this = $(this),
			$table = $this.closest( 'table' ),
			controlChecked = $this.prop('checked'),
			toggle = event.shiftKey || $this.data('toggle');
	...
	(function(){
		var button, input, form = $('form.upload-form');
		if ( ! form.length )
			return;
	...
	function pinMenu( event ) {
		...
		if ( isIOS || isIE8 || $adminmenu.data( 'responsive' ) ) {
			return;
		}
	...
	if ( ! isIOS ) {
		...
		$document.on( 'tinymce-editor-init.pin-menu', function( event, editor ) {
			editor.on( 'autoresize', resetHeights );
		});
	}
	...
	window.wpResponsive = {
		init: function() {
			...
			$document.on( 'responsive-activate.responsive', function() {
				self.activate();
			}).on( 'responsive-deactivate.responsive', function() {
				self.deactivate();
			});
			...
			$( '#admin-bar-menu-toggle' ).on( 'click.responsive', function( event ) {
				event.preventDefault();
				$wpwrap.toggleClass( 'responsive-open' );
				if ( $wpwrap.hasClass( 'responsive-open' ) ) {
			...
			// Add menu events
			$adminmenu.on( 'click.responsive', 'li.has-submenu > a', function( event ) {
				if ( ! $adminmenu.data('responsive') ) {
					return;
				}
			...
			self.trigger();
			$document.on( 'window-resized.responsive', $.proxy( this.trigger, this ) );
		...
		activate: function() {
			...
			$adminmenu.data( 'responsive', 1 );
			this.disableSortables();
		},

		deactivate: function() {
			setPinMenu();
			$adminmenu.removeData('responsive');
		...
		trigger: function() {
			...
			if ( width <= 782 ) {
				if ( ! wpResponsiveActive ) {
					$document.trigger( 'responsive-activate' );
					wpResponsiveActive = true;
				}
			} else {
				if ( wpResponsiveActive ) {
					$document.trigger( 'responsive-deactivate' );
					wpResponsiveActive = false;
				}
			}
		...
		enableOverlay: function() {
			if ( $overlay.length === 0 ) {
				$overlay = $( '<div id="responsive-overlay"></div>' )
			...
			$toolbarPopups.on( 'click.responsive', function() {
				$overlay.show();
			});
		},

		disableOverlay: function() {
			$toolbarPopups.off( 'click.responsive' );
			$overlay.hide();
		},
	...
	$document.on( 'window-resized.pin-menu postboxes-columnchange.pin-menu postbox-toggled.pin-menu collapse-menu.pin-menu scroll-start.pin-menu', setPinMenu );
});
...
( function() {
	...
	$(window).on( 'resize.fire-once', fireOnce );
}());
