...
setCommentsList = function() {
	...
	delBefore = function( settings, list ) {
		var note, id, el, n, h, a, author,
			action = false,
			wpListsData = $( settings.target ).attr( 'data-lists' );
		...
		if ( action ) {
			...
			a.attr('data-lists', 'delete:the-comment-list:comment-' + id + '::un' + action + '=1');
	...
	refillTheExtraList = function(ev) {
		...
		if (ev) {
			theExtraList.empty();
			args.number = Math.min(8, per_page); // see WP_Comments_List_Table::prepare_items() @ class-comments-list-table.php
		} else {
			args.number = 1;
	...
	theList = $('#the-comment-list').wpList( { alt: '', delBefore: delBefore, dimAfter: dimAfter, delAfter: delAfter, addColor: 'none' } )
		.bind('wpListDelEnd', function(e, s){
			var wpListsData = $(s.target).attr('data-lists'), id = s.element.replace(/[^0-9]+/g, '');
...
$(document).ready(function(){
	...
	if ( typeof $.table_hotkeys != 'undefined' ) {
		...
		toggle_all = function() {
			$('#cb-select-all-1').data( 'toggle', 1 ).trigger( 'click' ).removeData( 'toggle' );
		};
