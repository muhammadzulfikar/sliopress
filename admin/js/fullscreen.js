...
( function( $, window ) {
	...
	toggleUI = api.toggleUI = function( show ) {
		...
		if ( ! $body.hasClass('dfw-show-ui') || show === 'show' ) {
			$body.addClass('dfw-show-ui');
		...
		s.$dfwWrap.parents().each( function( i, parent ) {
			...
			if ( add ) {
				if ( parent.style.position ) {
					$parent.data( 'dfw-css-position', parent.style.position );
				}

				$parent.css( 'position', 'static' );
			} else {
				cssPosition = $parent.data( 'dfw-css-position' );
	...
	api.on = function() {
		...
		s.$dfwTextarea = $( '#' + id );
		s.$editorContainer = $dfwWrap.find( '.editor-container' );
	...
	api.save = function() {
		var $hidden = $('#hiddenaction'),
			oldVal = $hidden.val(),
			$spinner = $('#fullscreen-save .spinner'),
			$saveMessage = $('#fullscreen-save .fullscreen-saved-message'),
			$errorMessage = $('#fullscreen-save .fullscreen-error-message');

		$spinner.show();
		$errorMessage.hide();
		$saveMessage.hide();
		$hidden.val('fullscreen-save-post');
		...
		$.ajax({
			...
			if ( response && response.success ) {
				...
				if ( response.data && response.data.last_edited ) {
					$('#fullscreen-save input').attr( 'title',  response.data.last_edited );
				}
			} else {
				$errorMessage.show();
			}
	...
	api.dfwWidth = function( pixels, total ) {
		...
		if ( ! pixels ) {
			...
			width = $('#fullscreen-body').data('theme-width') || 800;
	...
	ps.subscribe( 'show', function() {
		...
		if ( title ) {
			$('#fullscreen-save input').attr( 'title', title );
		}
	});
	...
	ps.subscribe( 'showing', function() {
		...
		s.$dfwWrap.addClass( 'fullscreen-wrap' );

		if ( s.$dfwTitle ) {
			s.$dfwTitle.after( '<span id="fullscreen-title-placeholder">' );
			s.$dfwWrap.prepend( s.$dfwTitle.addClass('fullscreen-title') );
		}
		...
		if ( 'ontouchstart' in window ) {
			...
			api.dfwWidth( $( '#fullscreen-body' ).data('dfw-width') || 800, true );
		}
	...
	ps.subscribe( 'hide', function() { // This event occurs before the overlay blocks DFW.
		...
		s.$dfwTextarea.unbind('.dfw-resize');
	});
	...
	ps.subscribe( 'hiding', function() { // This event occurs while the overlay blocks the DFW UI.
		...
		if ( s.$dfwTitle ) {
			$( '#fullscreen-title-placeholder' ).before( s.$dfwTitle.removeClass('fullscreen-title').css( 'width', '' ) ).remove();
		}

		s.$dfwWrap.removeClass( 'fullscreen-wrap' );
	...
	api.refreshButtons = function( fade ) {
		if ( s.mode === 'html' ) {
			$('#fullscreen-mode-bar').removeClass('tmce-mode').addClass('html-mode')
				.find('a').removeClass( 'active' ).filter('.fullscreen-mode-html').addClass( 'active' );

			if ( fade ) {
				$('#fullscreen-button-bar').fadeOut( 150, function(){
					$(this).addClass('html-mode').fadeIn( 150 );
				});
			} else {
				$('#fullscreen-button-bar').addClass('html-mode');
			}
		} else if ( s.mode === 'tinymce' ) {
			$('#fullscreen-mode-bar').removeClass('html-mode').addClass('tmce-mode')
				.find('a').removeClass( 'active' ).filter('.fullscreen-mode-tinymce').addClass( 'active' );

			if ( fade ) {
				$('#fullscreen-button-bar').fadeOut( 150, function(){
					$(this).removeClass('html-mode').fadeIn( 150 );
				});
			} else {
				$('#fullscreen-button-bar').removeClass('html-mode');
			}
		}
	};
	...
	api.ui = {
		init: function() {
			...
			s.$statusbar = $('#fullscreen-status');
			s.hasTinymce = typeof tinymce !== 'undefined';

			if ( ! s.hasTinymce )
				$('#fullscreen-mode-bar').hide();
			...
			$( window ).on( 'keydown.fullscreen', function( event ) {
			...
			// Bind buttons
			$('#fullscreen-buttons').on( 'click.fullscreen', 'button', function( event ) {
				...
				if ( command === 'media-library' && typeof wp !== 'undefined' && wp.media && wp.media.editor ) {
	...
	api.bind_resize = function() {
		s.$dfwTextarea.on( 'keydown.dfw-resize click.dfw-resize paste.dfw-resize', function() {
			api.resizeTextarea();
		});
	};
