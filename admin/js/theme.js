...
themes.view.Appearance = wp.Backbone.View.extend({
	...
	search: function() {
		...
		this.searchContainer
			.append( $.parseHTML( '<label class="screen-reader-text" for="filter-search-input">' + l10n.search + '</label>' ) )
			.append( view.el );
	},
...
themes.view.Details = wp.Backbone.View.extend({
	...
	containFocus: function( $el ) {
		...
		$el.on( 'keydown.themes', function( event ) {
...
themes.view.Preview = themes.view.Details.extend({
	...
	className: 'full-overlay expanded',
...
themes.view.Installer = themes.view.Appearance.extend({
	...
	// in new location
	searchContainer: $( '.filter .search-form' ),
...
themes.RunInstaller = {
	...
	routes: function() {
		...
		themes.router.on( 'route:search', function() {
			$( '.filter-search' ).focus().trigger( 'keyup' );
		});
