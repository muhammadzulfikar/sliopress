...
<table id="list-table" style="display: none;">
	...
	<tbody id="the-list" data-lists="list:meta">
	<tr><td></td></tr>
	</tbody>
...
function _list_meta_row( $entry, &$count ) {
	...
	$r .= "\n\t\t<div class='submit'>";
	$r .= get_submit_button( __( 'Delete' ), 'deletemeta small', "deletemeta[{$entry['meta_id']}]", false, array( 'data-lists' => "delete:the-list:meta-{$entry['meta_id']}::_ajax_nonce=$delete_nonce" ) );
	$r .= "\n\t\t";
	$r .= get_submit_button( __( 'Update' ), 'updatemeta small', "meta-{$entry['meta_id']}-submit", false, array( 'data-lists' => "add:the-list:meta-{$entry['meta_id']}::_ajax_nonce-add-meta=$update_nonce" ) );
	$r .= "</div>";
...
<tr><td colspan="2">
<div class="submit">
<?php submit_button( __( 'Add Custom Field' ), 'secondary', 'addmeta', false, array( 'id' => 'newmeta-submit', 'data-lists' => 'add:the-list:newmeta' ) ); ?>
</div>
...
?>
<form enctype="multipart/form-data" id="import-upload-form" method="post" class="upload-form" action="<?php echo esc_url( wp_nonce_url( $action, 'import-upload' ) ); ?>">
<p>
...
?>
<body<?php if ( isset($GLOBALS['body_id']) ) echo ' id="' . $GLOBALS['body_id'] . '"'; ?> class="admin core-ui no-js iframe <?php echo $admin_body_classes . ' ' . $admin_body_class; ?>">
<script type="text/javascript">
//<![CDATA[
(function(){
	...
	var testCompression = {
		get : function(test) {
			...
			if (x) {
				...
				x.open('GET', ajaxurl + '?action=compression-test&test='+test+'&'+(new Date()).getTime(), true);
				x.send('');
			}
		},
...
function _wp_admin_html_begin() {
	...
	$admin_html_class = ( is_admin_bar_showing() ) ? 'toolbar' : '';
...
final class WP_Internal_Pointers {
	 ...
	 * @param array  $args Arguments to be passed to the pointer JS (see pointer.js).
	 */
	private static function print_js( $pointer_id, $selector, $args ) {
		...
		(function($){
			...
			if ( options.position && options.position.defer_loading )
				$(window).bind( 'load.pointers', setup );
			else
	...
	public static function pointer_wp360_locks() {
		...
		self::print_js( 'wp360_locks', 'tr.locked .locked-indicator', array(
			'content' => $content,
	...
	public static function pointer_wp410_dfw() {
		...
		self::print_js( 'wp410_dfw', '#content-wrap', array(
			'content' => $content,
			'position' => $position,
		) );
	}
...
function convert_to_screen( $hook_name ) {
	if ( ! class_exists( 'WP_Screen' ) ) {
		_doing_it_wrong( 'convert_to_screen(), add_meta_box()', __( "Likely direct inclusion of admin/includes/template.php in order to use add_meta_box(). This is very wrong. Hook the add_meta_box() call into the add_meta_boxes action instead." ), '3.3' );
