...
class WP_Plugins_List_Table extends WP_List_Table {
	...
	public function prepare_items() {
		...
		if ( ! is_multisite() || ( $screen->in_admin( 'network' ) && current_user_can( 'manage_network_plugins' ) ) ) {
			...
			/** This action is documented in admin/includes/class-plugins-list-table.php */
	...
	protected function extra_tablenav( $which ) {
		...
		elseif ( 'top' == $which && 'dropins' == $status )
			echo '<p>' . sprintf( __( 'Drop-ins are advanced plugins in the <code>%s</code> directory that replace SlioPress functionality when present.' ), str_replace( ABSPATH, '', WP_CONTENT_DIR ) ) . '</p>';
	...
	public function single_row( $item ) {
		...
		if ( 'mustuse' == $context ) {
			...
			if ( true === ( $dropins[ $plugin_file ][1] ) ) { // Doesn't require a constant
				...
				$is_active = false;
				$description = '<p><strong>' . $dropins[ $plugin_file ][0] . ' <span class="attention">' . __('Inactive:') . '</span></strong> ' . sprintf( __( 'Requires <code>%s</code> in <code>config.php</code>.' ), "define('" . $dropins[ $plugin_file ][1] . "', true);" ) . '</p>';
			}
