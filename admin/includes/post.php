...
function _fix_attachment_links( $post ) {
	...
	foreach ( $link_matches[1] as $key => $value ) {
		if ( !strpos($value, '?attachment_id=') || !strpos($value, 'att-')
			|| !preg_match( '/href=(["\'])[^"\']*\?attachment_id=(\d+)[^"\']*\\1/', $value, $url_match )
			|| !preg_match( '/rel=["\'][^"\']*att-(\d+)/', $value, $rel_match ) )
				continue;
...
function get_sample_permalink_html( $id, $new_title = null, $new_slug = null ) {
	...
	if ( isset( $view_post ) ) {
		if( 'draft' == $post->post_status ) {
			...
			$return .= "<span id='view-post-btn'><a href='" . esc_url( $preview_link ) . "' class='button button-small' target='preview-{$post->ID}'>$view_post</a></span>\n";
		} else {
...
function _admin_notice_post_locked() {
	...
	if ( $locked ) {
		...
		$tab_last = $override ? '' : ' tab-last';

		?>
		<div class="post-locked-message">
		<div class="post-locked-avatar"><?php echo get_avatar( $user->ID, 64 ); ?></div>
		<p class="currently-editing tab-first" tabindex="0">
		<?php
		...
		if ( $override ) {
			?>
			<a class="button button-primary tab-last" href="<?php echo esc_url( add_query_arg( 'get-post-lock', '1', get_edit_post_link( $post->ID, 'url' ) ) ); ?>"><?php _e('Take over'); ?></a>
			<?php
		}
		...
		<div class="post-taken-over">
			...
			?>
			<p><a class="button button-primary tab-last" href="<?php echo esc_url( $sendback ); ?>"><?php echo $sendback_text; ?></a></p>
		</div>
		<?php
	}
