...
function wp_check_locked_posts( $response, $data, $screen_id ) {
	...
	if ( array_key_exists( 'check-locked-posts', $data ) && is_array( $data['check-locked-posts'] ) ) {
		foreach ( $data['check-locked-posts'] as $key ) {
	...
	if ( ! empty( $checked ) )
		$response['check-locked-posts'] = $checked;
...
function wp_refresh_post_lock( $response, $data, $screen_id ) {
	if ( array_key_exists( 'refresh-post-lock', $data ) ) {
		...
		$response['refresh-post-lock'] = $send;
	}
...
function wp_refresh_post_nonces( $response, $data, $screen_id ) {
	if ( array_key_exists( 'refresh-post-nonces', $data ) ) {
		$received = $data['refresh-post-nonces'];
		$response['refresh-post-nonces'] = array( 'check' => 1 );
		...
		if ( 2 === wp_verify_nonce( $received['post_nonce'], 'update-post_' . $post_id ) ) {
			$response['refresh-post-nonces'] = array(
				'replace' => array(
