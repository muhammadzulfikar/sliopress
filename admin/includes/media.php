<?php
/**
 * SlioPress Administration Media API.
 *
 * @package SlioPress
...
function get_image_send_to_editor($id, $caption, $title, $align, $url='', $rel = false, $size='medium', $alt = '') {
	...
	$rel = $rel ? ' rel="attachment att-' . esc_attr($id).'"' : '';
...
?>
<title><?php bloginfo('name') ?> &rsaquo; <?php _e('Uploads'); ?> &#8212; <?php _e('SlioPress'); ?></title>
<?php
...
<body<?php if ( isset($GLOBALS['body_id']) ) echo ' id="' . $GLOBALS['body_id'] . '"'; ?> class="core-ui no-js">
<script type="text/javascript">
...
function media_buttons($editor_id = 'content') {
	...
	$img = '<span class="media-buttons-icon"></span> ';
	...
	if ( isset($send_id) ) {
		...
		if ( !empty($attachment['url']) ) {
			...
			if ( strpos($attachment['url'], 'attachment_id') || get_attachment_link($send_id) == $attachment['url'] )
				$rel = " rel='attachment att-" . esc_attr($send_id) . "'";
...
function get_media_item( $attachment_id, $args = null ) {
	...
	if ( 'image' == $type && $calling_post_id && current_theme_supports( 'post-thumbnails', get_post_type( $calling_post_id ) )
		...
		$thumbnail = "<a class='post-thumbnail' id='post-thumbnail-" . $attachment_id . "' href='#' onclick='WPSetAsThumbnail(\"$attachment_id\", \"$ajax_nonce\");return false;'>" . esc_html__( "Use as featured image" ) . "</a>";
	}
...
function media_upload_html_bypass() {
	...
	<p class="upload-html-bypass hide-if-no-js">
	   <?php _e('You are using the browser&#8217;s built-in file uploader. The SlioPress uploader includes multiple file selection and drag and drop capability. <a href="#">Switch to the multi-file uploader</a>.'); ?>
	</p>
	<?php
}
