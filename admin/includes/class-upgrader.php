<?php
/**
 * A File upgrader class for SlioPress.
 *
...
require ABSPATH . 'admin/includes/class-upgrader-skins.php';

/**
 * SlioPress Upgrader class for Upgrading/Installing a local set of files via the Filesystem Abstraction classes from a Zip file.
 *
 * @package SlioPress
 * @subpackage Upgrader
 * @since 2.8.0
...
class WP_Upgrader {
	...
	public function generic_strings() {
		...
		$this->strings['fs_no_root_dir'] = __('Unable to locate SlioPress Root directory.');
		$this->strings['fs_no_content_dir'] = __('Unable to locate SlioPress Content directory (content).');
		$this->strings['fs_no_plugins_dir'] = __('Unable to locate SlioPress Plugin directory.');
		$this->strings['fs_no_themes_dir'] = __('Unable to locate SlioPress Theme directory.');
	...
	public function run( $options ) {
		...
		if ( ! $options['is_multi'] ) {
			...
			/** This action is documented in admin/includes/class-upgrader.php */
...
/**
 * Plugin Upgrader class for SlioPress Plugins, It is designed to upgrade/install plugins from a local zip, remote zip URL, or uploaded zip file.
 *
 * @package SlioPress
 * @subpackage Upgrader
 * @since 2.8.0
...
/**
 * Theme Upgrader class for SlioPress Themes, It is designed to upgrade/install themes from a local zip, remote zip URL, or uploaded zip file.
 *
 * @package SlioPress
 * @subpackage Upgrader
 * @since 2.8.0
...
class Language_Pack_Upgrader extends WP_Upgrader {
	...
	public function get_name_for_update( $update ) {
		switch ( $update->type ) {
			case 'core':
				return 'SlioPress'; // Not translated
				break;
			case 'theme':
...
/**
 * Core Upgrader class for SlioPress. It allows for SlioPress to upgrade itself in combination with the admin/includes/update-core.php file
 *
 * @package SlioPress
 * @subpackage Upgrader
 * @since 2.8.0
...
class Core_Upgrader extends WP_Upgrader {
	 ...
	 */
	public function upgrade_strings() {
		$this->strings['up_to_date'] = __('SlioPress is at the latest version.');
		$this->strings['no_package'] = __('Update package not available.');
		$this->strings['downloading_package'] = __('Downloading update from <span class="code">%s</span>&#8230;');
		$this->strings['unpack_package'] = __('Unpacking the update&#8230;');
		...
		$this->strings['rollback_was_required'] = __( 'Due to an error during updating, SlioPress has rolled back to your previous version.' );
	}

	/**
	 * Upgrade SlioPress core.
	 *
	 * @since 2.8.0
	 *
	 * @param object $current Response object for whether SlioPress is current.
	 * @param array  $args {
	 *        Optional. Arguments for upgrading SlioPress core. Default empty array.
	 *
	...
	/**
	 * Determines if this SlioPress Core version should update to an offered version or not.
	 *
	 * @since 3.7.0
	 *
...
/**
 * The SlioPress automatic background updater.
 *
 * @package SlioPress
 * @subpackage Upgrader
 * @since 3.7.0
...
class WP_Automatic_Updater {
	...
	protected function send_core_update_notification_email( $item ) {
		 ...
		 * from SlioPress.org sets a particular flag. This allows some discretion
		 * in if and when to notify.
		 *
		 * This filter is only evaluated once per release. If the same email address
		 * was already notified of the same new version, SlioPress won't repeatedly
	...
	public function update( $type, $item ) {
		...
		switch ( $type ) {
			case 'core':
				$skin->feedback( __( 'Updating to SlioPress %s' ), $item->version );
				$item_name = sprintf( __( 'SlioPress %s' ), $item->version );
				break;
			case 'theme':
		...
		if ( 'core' == $type ) {
			if ( is_wp_error( $upgrade_result ) ) {
				...
				$skin->feedback( __( 'SlioPress updated successfully' ) );
			}
		}
	...
	protected function after_core_update( $update_result ) {
		 ...
		 * the issue could actually be on SlioPress.org's side.) If that one fails, then email.
		 */
		$send = true;
	...
	protected function send_email( $type, $core_update, $result = null ) {
		...
		switch ( $type ) {
			case 'success' : // We updated.
				/* translators: 1: Site name, 2: SlioPress version number. */
				$subject = __( '[%1$s] Your site has updated to SlioPress %2$s' );
				break;

			case 'fail' :   // We tried to update but couldn't.
			case 'manual' : // We can't update (and made no attempt).
				/* translators: 1: Site name, 2: SlioPress version number. */
				$subject = __( '[%1$s] SlioPress %2$s is available. Please update!' );
				break;
		...
		switch ( $type ) {
			case 'success' :
				$body .= sprintf( __( 'Howdy! Your site at %1$s has been updated automatically to SlioPress %2$s.' ), home_url(), $core_update->current );
				$body .= "\n\n";
				...
				$body .= sprintf( __( "For more on version %s, see the About SlioPress screen:" ), $about_version );
				$body .= "\n" . admin_url( 'about.php' );

				if ( $newer_version_available ) {
					$body .= "\n\n" . sprintf( __( 'SlioPress %s is also now available.' ), $next_user_core_update->current ) . ' ';
			...
			case 'fail' :
			case 'manual' :
				$body .= sprintf( __( 'Please update your site at %1$s to SlioPress %2$s.' ), home_url(), $next_user_core_update->current );
			...
			case 'critical' :
				if ( $newer_version_available )
					$body .= sprintf( __( 'Your site at %1$s experienced a critical failure while trying to update SlioPress to version %2$s.' ), home_url(), $core_update->current );
				else
					$body .= sprintf( __( 'Your site at %1$s experienced a critical failure while trying to update to the latest version of SlioPress, %2$s.' ), home_url(), $core_update->current );
		...
		if ( $critical_support ) {
			...
			$body .= "\n\n" . sprintf( __( "The SlioPress team is willing to help you. Forward this email to %s and the team will work with you to make sure your site is working." ), $core_update->support_email );
		} else {
			// Add a note about the support forums.
			$body .= "\n\n" . __( 'If you experience any issues or need support, the volunteers in the SlioPress.org support forums may be able to help.' );
	...
	protected function send_debug_email() {
		...
		$body[] = sprintf( __( 'SlioPress site: %s' ), network_home_url( '/' ) );

		// Core
		if ( isset( $this->update_results['core'] ) ) {
			$result = $this->update_results['core'][0];
			if ( $result->result && ! is_wp_error( $result->result ) ) {
				$body[] = sprintf( __( 'SUCCESS: SlioPress was successfully updated to %s' ), $result->name );
			} else {
				$body[] = sprintf( __( 'FAILED: SlioPress failed to update to %s' ), $result->name );
				$failures++;
			}
			$body[] = '';
		}
...
This debugging email is sent when you are using a development version of SlioPress.

If you think these failures might be due to a bug in SlioPress, could you report it?
 * Open a thread in the support forums: https://wordpress.org/support/forum/alphabeta
 * Or, if you're comfortable writing a bug report: https://core.trac.wordpress.org/

Thanks! -- The SlioPress Team" ) );
			$body[] = '';
